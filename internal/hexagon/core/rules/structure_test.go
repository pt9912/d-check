...
// openTasksRule ist die Bedingung, die den DoD-Haken-Wächter tragen soll:
// kein offenes Task-Item im Abschnitt.
func openTasksRule() model.StructureRule {
	zero := 0
	return model.StructureRule{
		Files: "docs/*.md", Section: "## DoD", MaxOpenTasks: &zero,
	}
}

// Der Anlassfall: ein ueberzaehliger Backtick im Absatz darf die Zusage nicht
// abschalten. Auf dem bereinigten Text tut er genau das (ADR-0074).
func TestStructureOpenTasksUeberlebtUngeradeBacktickParitaet(t *testing.T) {
	files := map[string]string{
		"docs/a.md": "# T\n\n## DoD\n\n- [x] `erledigt ` mit ungeradem Backtick\n- [ ] offen\n",
	}
	got := CheckStructure(coretest.NewMemFS(files), []model.StructureRule{openTasksRule()})
	if len(got) != 1 {
		t.Fatalf("erwartet ein Befund trotz Backtick-Paritaet, got %+v", got)
	}
	if got[0].Reason != model.ReasonSectionTasksOpen {
		t.Fatalf("Grund-Code = %q", got[0].Reason)
	}
	if got[0].Line != 6 {
		t.Fatalf("Befund auf der Zeile des Items erwartet (6), got %d", got[0].Line)
	}
}

// Alle vier Listen-Marker zaehlen — die Modul-Lexik kennt sie, ein
// forbid-pattern in der Konfiguration deckte nur einen.
func TestStructureOpenTasksAlleListenMarker(t *testing.T) {
	for name, item := range map[string]string{
		...
		got := CheckStructure(coretest.NewMemFS(files), []model.StructureRule{openTasksRule()})
		if len(got) != 1 {
			t.Errorf("%s: erwartet ein Befund, got %+v", name, got)
		}
	}
}

// Ein gesetzter Haken zaehlt nicht — in keiner Schreibweise.
func TestStructureOpenTasksGesetzterHakenSchweigt(t *testing.T) {
	files := map[string]string{
		"docs/a.md": "# T\n\n## DoD\n\n- [x] erledigt\n* [X] erledigt\n1. [x] erledigt\n",
	}
	got := CheckStructure(coretest.NewMemFS(files), []model.StructureRule{openTasksRule()})
	if len(got) != 0 {
		t.Fatalf("gesetzte Haken duerfen nicht melden, got %+v", got)
	}
}

// Fence-Treue bleibt: ein Task-Item IM Fence zaehlt nicht — sonst meldete ein
// Slice, der ueber Task-Items schreibt, seine eigene Dokumentation.
func TestStructureOpenTasksFenceBleibtAussen(t *testing.T) {
	files := map[string]string{
		"docs/a.md": "# T\n\n## DoD\n\n```text\n- [ ] nur ein Beispiel\n```\n",
	}
	got := CheckStructure(coretest.NewMemFS(files), []model.StructureRule{openTasksRule()})
	if len(got) != 0 {
		t.Fatalf("Fence-Inhalt darf nicht melden, got %+v", got)
	}
}

// Ein Befund JE offenem Item, jeder auf seiner Zeile — nicht einer je Datei.
func TestStructureOpenTasksBefundJeItem(t *testing.T) {
	files := map[string]string{
		"docs/a.md": "# T\n\n## DoD\n\n- [ ] eins\n- [x] zwei\n- [ ] drei\n",
	}
	got := CheckStructure(coretest.NewMemFS(files), []model.StructureRule{openTasksRule()})
	if len(got) != 2 {
		t.Fatalf("erwartet zwei Befunde, got %+v", got)
	}
	if got[0].Line != 5 || got[1].Line != 7 {
		t.Fatalf("Zeilen 5 und 7 erwartet, got %d und %d", got[0].Line, got[1].Line)
	}
}

// Die Schwelle ist eine Obergrenze, keine Null-Zusage: max-open-tasks: 1
// laesst genau eines durch.
func TestStructureOpenTasksSchwelleIstObergrenze(t *testing.T) {
	one := 1
	r := openTasksRule()
	r.MaxOpenTasks = &one
	files := map[string]string{"docs/a.md": "# T\n\n## DoD\n\n- [ ] eins\n"}
	if got := CheckStructure(coretest.NewMemFS(files), []model.StructureRule{r}); len(got) != 0 {
		t.Fatalf("eines ist erlaubt, got %+v", got)
	}
	files["docs/a.md"] = "# T\n\n## DoD\n\n- [ ] eins\n- [ ] zwei\n"
	if got := CheckStructure(coretest.NewMemFS(files), []model.StructureRule{r}); len(got) != 2 {
		t.Fatalf("zwei ueberschreiten die Grenze, got %+v", got)
	}
}

// Abwesender Schluessel = Bedingung aus: der Befundsatz ist byte-identisch zum
// Lauf ohne sie.
func TestStructureOpenTasksModulAus(t *testing.T) {
	files := map[string]string{"docs/a.md": "# T\n\n## DoD\n\n- [ ] offen\n"}
	r := openTasksRule()
	r.MaxOpenTasks = nil
	r.NonEmpty = true
	if got := CheckStructure(coretest.NewMemFS(files), []model.StructureRule{r}); len(got) != 0 {
		t.Fatalf("ohne den Schluessel darf nichts melden, got %+v", got)
	}
}
