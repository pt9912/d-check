...
report on lines and do not know the marker. At `codepaths` and `ids` the marker counts only **outside inline code** — in backticks it is a mention; at `versions` and `diagrams` it stays raw — structurally for `diagrams`, as a named boundary for `versions` (it does read prose lines)
([`DC-FA-CODE-001`](spec/lastenheft.md#dc-fa-code-001--explizite-pfade-in-inline-code-modul-codepaths-opt-in),
