...
**Und drei benannte Grenzen sind der Preis dafür:** ein Unterstrich-Kompositum
...
Wahl fällt auf die Seite, die die häufigen Formen zählen lässt.
...
- **Eigenständige Nennung (was zählen muss):** Given ein Mitglied `x.md`, when ein Dokument es als `siehe x.md.`, als `x.md-Datei`, als `_x.md_` oder — unter `path` — als `../a/x.md` nennt, then **kein** Befund: Satzzeichen, Kompositum-Fuge, Kursivierung und relative Verlinkung sind keine Namensbestandteile. <!-- d-check:ignore (Illustrations-Pfade im Akzeptanzkriterium) -->
