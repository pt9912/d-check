...
- **Hilfe:** Given `d-check --help`, when aufgerufen, then Exit-Code 0 und die Nutzung auf stderr enthält die Synopsis mit `[pfad]`, die Pfad-Argument-Beschreibung, einen Verweis auf `--print-config` **und beide URLs des Benutzerhandbuchs** auf dem **Hauptzweig** (ohne Versionsangabe) — die gerenderte GitHub-Seite und die rohe `raw.githubusercontent.com`-Form.
...
[`DC-FA-DIST-001`](#dc-fa-dist-001--docker-image)) — sowie dreizehn
...
**hermetisch** ohne `RANGE`/`STAGED`), `doc-usage` (Aufruf und Optionen von
`d-check` selbst — der Hilfe-Modus aus
[`DC-FA-CLI-001`](#dc-fa-cli-001--aufruf-und-scan-wurzel) als Target, damit der
Konsument die Oberfläche des gepinnten Image erreicht, ohne sich den
`docker run`-Aufruf selbst zusammenzusetzen; Recipe-Echo unterdrückt, und die
Hilfe erscheint — wie beim direkten Aufruf — auf **stderr**, nicht auf stdout)
und `doc-help` (listet die
...
**Out-of-Scope:** Schreiben der `d-check.mk` (immer stdout); Einbetten des eigenen Image-**Digests** ins Binary (Henne-Ei — der Digest hasht das Binary selbst; der Konsument pinnt per `DCHECK_IMAGE`-Override); weitere Targets jenseits der gelisteten dreizehn (`doc-check`/`doc-trace`/`doc-complete`/`doc-doctor`/`doc-repair`/`doc-immutable`/`doc-commits`/`doc-planning`/`doc-tracked`/`doc-targets`/`doc-structure`/`doc-usage`/`doc-help` — Konsumenten komponieren weitere `gates` selbst); ein `help`-Target (Namens-Kollision mit dem Konsumenten — daher namespaced `doc-help`); Nicht-`@sha256:`-Digest-Formen in `DCHECK_DIGEST`; die Exit-Code-Semantik der RTM-Targets selbst (in [`DC-FA-CLI-011`](#dc-fa-cli-011--vollständigkeits-prüfung-als-opt-in-exit-code) bzw. [`DC-FA-CLI-009`](#dc-fa-cli-009--requirements-traceability-matrix) festgelegt); Nicht-Make-Build-Systeme.
...
**Die Härte kennt einen erklärten Ausnahmefall: `exempt-expect-count`** (int ≥ 0,
...
der Bestand ist gerade vollständig ausgenommen, und die Regel wartet auf den
ersten nicht ausgenommenen Abschnitt. **Weicht sie ab ⇒
`section-exempt-mismatch`**, und zwar in **beide** Richtungen und **unabhängig**
davon, ob noch Abschnitte übrig sind: eine erweiterte Aufzählung ohne
nachgezogene Zahl ist dieselbe Lücke wie eine veraltete. Er trägt einen
**eigenen** Grund-Code, obwohl die beiden Ventile darüber keinen tragen: die
Reparatur ist eine andere — *Aufzählung oder Zahl nachziehen* statt *Selektor
korrigieren*.

**Zwei Zusagen, die er NICHT trägt, und beide gehören genannt.** Die Zahl gilt
**je Datei**, nicht je Regel: eine Regel über einen Glob prüft sie gegen
**jede** getroffene Datei einzeln — für einen Bestand, der in **einer** Datei
lebt, ist das der Normalfall, für einen Glob über viele Dateien ist es fast nie
die gemeinte Aussage. Und der Befund **bricht die Datei ab**: was hinter dem
Mismatch stünde, wird nicht mehr geprüft. Eine falsche Zahl verdeckt also den
Rest, bis sie stimmt.

**Die Trennung, um die es geht:** trifft schon `section-pattern` nichts, bleibt
es `section-missing` — auch mit gesetzter Zahl. Ein Konfigurationsdefekt und ein
Bestandszustand sahen vorher gleich aus. **Eine deklarierte Null bedeutet
etwas** (*„das Muster soll heute noch nichts treffen"*) und ist von einem
abwesenden Schlüssel unterscheidbar.
...
**ohne** `open-tasks-require-marker` (dieselbe halbe Aktivierung, zwei Stufen
tief); explizit
...
- **Bedingte Pflicht-Marke, Erlaubnis (`open-tasks-require-marker`):** Given `max-open-tasks: 0`, `open-tasks-require-marker: "Gegenstand"` und einen Abschnitt mit **drei** offenen Task-Items sowie der Marke `**Gegenstand:**` im bereinigten Text, when `d-check --enable structure` läuft, then **kein** Befund — die drei `section-tasks-open` entfallen vollständig.
- **Bedingte Pflicht-Marke, Pflicht:** Given dieselbe Regel und denselben Abschnitt **ohne** die Marke, when der Lauf endet, then **genau ein** `section-open-tasks-marker-missing` auf der Überschriftszeile — **nicht** die drei `section-tasks-open`, die ohne den Schlüssel entstünden.
- **Bedingte Pflicht-Marke, Normalfall unberührt:** Given dieselbe Regel und einen Abschnitt **ohne** offene Task-Items (alle gehakt), when der Lauf endet, then kein Befund — unabhängig davon, ob die Marke dasteht; die Kopplung greift nur, wenn `max-open-tasks` selbst bereits meldet.
- **Marken-Suche in einem anderen Abschnitt (`open-tasks-require-marker-section`):** Given dieselbe Regel plus `open-tasks-require-marker-section: '^#{1,3} [0-9]+\. Closure-Notiz'`, einen DoD-Abschnitt mit **zwei** offenen Task-Items und **keiner** Marke darin, aber einem **separaten** Abschnitt „## 7. Closure-Notiz" mit `**Gegenstand:** entfallen: …`, when `d-check --enable structure` läuft, then **kein** Befund — die Marke gilt als vorhanden, weil sie im benannten Abschnitt steht, nicht im gezählten. Given denselben Aufbau **ohne** `open-tasks-require-marker-section`, then **ein** `section-open-tasks-marker-missing` — der Vorzustand vor dieser Erweiterung, der Baseline `v6.9.0`s eigene Ziel-Form (Marke ausschließlich in einem eigenen Closure-Abschnitt) fälschlich als „Marke fehlt" liest.
- **Marken-Suche, fehlender benannter Abschnitt:** Given dieselbe Kopplung, aber eine Datei **ohne** einen zu `open-tasks-require-marker-section` passenden Abschnitt, when der Lauf endet, then gilt die Marke als fehlend — `section-open-tasks-marker-missing`, nicht Exit 2 und keine stille Ausnahme.
- **Marken-Suche, abweichende Nummerierung:** Given `open-tasks-require-marker-section: '^#{1,3} [0-9]+\. Closure-Notiz'` und einen Abschnitt „## 9. Closure-Notiz (nach `done/`)" mit der Marke, when der Lauf endet, then wird sie gefunden — das Muster ankert nur den Präfix, nicht die vollständige Zeile.
- **Bedingte Pflicht-Marke, abwesender Schlüssel:** Given dieselbe Regel **ohne** `open-tasks-require-marker`, when derselbe Abschnitt ohne Marke geprüft wird, then die **drei** `section-tasks-open`-Befunde wie vor dieser Fähigkeit — byte-identisches Verhalten.
...
Geprüft wird das Vorkommen einer Zeichenkette im Dokument-Text — und zwar als
**eigenständige Nennung**, nicht als beliebige Teilzeichenkette. **Die Grenze
ist asymmetrisch und eng geschnitten**, weil eine weite Grenze das Gegenteil
des Gewollten tut und *erwähnte* Artefakte meldet: **links** blockiert ein
Buchstabe oder eine Ziffer (jeder Schrift) sowie `-` und `.`; **rechts**
blockiert nur ein Buchstabe oder eine Ziffer.

**Was damit erlaubt bleibt, ist der Regelfall in Prosa** — alle vier Formen am
Bestand gemessen: `/` links trägt die `../`-relative Verlinkung (ohne sie
meldete ein Adopter mit dem üblichen Layout seine **gesamte** Soll-Menge als
Befund), `_` beidseitig die Kursivierung, `.` rechts den Satz-Schlusspunkt und
`-` rechts das deutsche Kompositum.

**Der Preis ist eine offene Klasse, und drei Vertreter stehen hier:** ein Unterstrich-Kompositum
deckt das kürzere Mitglied, eine angehängte Endung ebenso, und ein fremdes
Pfad-Präfix deckt das Mitglied darunter. Eine rein textuelle Regel kann
Namensteil und Satzzeichen nicht trennen, wo dasselbe Zeichen beides ist; die
Wahl fällt auf die Seite, die die häufigen Formen zählen lässt. Was die Klasse **schließt**, ist nicht ihre Aufzählung, sondern der Kollisions-Wächter unten: Der gefährliche Fall — ein Mitglied, das ein anderes deckt — bricht ab, statt still grün zu werden.
...
überall), ein Pfad ist im Repo eindeutig. **Und die Messung stützt ihn:** über
zwölf Artefakte aus drei Repos weichen die Formen **einmal** ab — ein
`harness/README.md` ist per Pfad nicht erwähnt, per Dateiname doch, und die
Nennung meint dort die Projekt-Wurzel-`README.md`. Der einzige gemessene
Unterschied ist damit ein **Fehlalarm der laxen Form**, und genau die
Kollision, gegen die der Default steht.
...
Default bleibt der strengere.

**Die Kollisions-Vorbedingung prüft das Modul, nicht der Konfigurierende.**
Steckt der Suchbegriff eines Mitglieds im Suchbegriff eines anderen — als
eigenständige Nennung, nach derselben Grenz-Regel —, dann kann dieses Mitglied
**nie** als unerwähnt gemeldet werden: Die Nennung des längeren Geschwisters
deckt es. Das ist **Exit 2** und kein Befund, mit Nennung **beider**
Mitglieder. Gewöhnliche Bestände treffen das: `x.md`/`x.md.bak`,
`x.yml`/`x.yml.example`, `x.sh`/`x.sh.in`, und unter `basename` zusätzlich
derselbe Dateiname in zwei Verzeichnissen. **Warum das Modul und nicht der
Mensch:** Die Bedingung hängt an der Grenz-Regel, und die ist Produkt-Interna —
eine von Hand nachzurechnende Vorbedingung ist bei jeder Änderung der Grenze
still falsch. Sie war es zweimal.
...
sagt, worüber er geurteilt hat: `N von M Artefakt(en) erwähnt`, über `D`
Dokument(e) — damit ein Lauf ohne Befund benennt, worüber er nichts gefunden
hat. **In beiden Ausgabe-Formen**, und das ist eine Festlegung, keine
Selbstverständlichkeit: im Default-Modus als Teil der Zusammenfassung auf
stderr, unter `--json`/`--yaml` als zusätzliche Felder des `summary`-Objekts —
[`DC-FA-CLI-004`](#dc-fa-cli-004--ausgabeformate) fordert dort **mindestens**
`filesChecked` und `findingCount` und lässt weitere ausdrücklich zu. Eine
Zusammenfassung, die es nur auf stderr gäbe, wäre unter `--json` nicht
vorhanden, und das Kriterium darunter prüfte einen Modus mit und einen ohne.
Sie gehört **nicht** ins `message`-Feld eines Befundes: dessen **Wortlaut**
sagt derselbe Vertrag ausdrücklich nicht zu, und ein Akzeptanzkriterium darf
nicht prüfen, was der Vertrag nicht zusagt.
...
- **Bezugsmenge im sauberen Lauf:** Given eine Soll-Menge, deren Mitglieder alle erwähnt sind, when der Lauf erfolgt, then nennt die Zusammenfassung die Bezugsmenge (`N von M Artefakt(en) erwähnt`, über `D` Dokument(e)) — im Default-Modus auf stderr, unter `--json`/`--yaml` als Felder des `summary`-Objekts. Ein Lauf ohne Befund sagt damit in **beiden** Formen, worüber er nichts gefunden hat.
...
- **Eigenständige Nennung (linke Grenze):** Given ein Mitglied `test.md` und ein Dokument, das **nur** `image-test.md` nennt, when der Lauf unter `basename` erfolgt, then **ein** Befund für `test.md` — eine Teilzeichenketten-Nennung deckt nicht.
- **Eigenständige Nennung (was zählen muss):** Given ein Mitglied `a/x.md`, when ein Dokument es als `siehe a/x.md.`, als `a/x.md-Datei`, als `_a/x.md_` oder als `../a/x.md` nennt, then **kein** Befund: Satzzeichen, Kompositum-Fuge, Kursivierung und relative Verlinkung sind keine Namensbestandteile. <!-- d-check:ignore (Illustrations-Pfade im Akzeptanzkriterium) -->
- **Grenze ist runen-basiert:** Given ein Dokument, das `Ätest.md` nennt, when unter `basename` gegen das Mitglied `test.md` geprüft wird, then **ein** Befund — ein Nicht-ASCII-Buchstabe links blockiert wie jeder andere.
- **`scan.ignore` prunt beide Mengen:** Given ein Artefakt in einem per `scan.ignore` ausgenommenen Teilbaum und ein Glob, das ihn treffen würde, when der Lauf erfolgt, then ist es **kein** Mitglied — ein bewusst ausgenommener Fremdbaum kommt nicht über die Soll-Menge zurück.
- **Negative (unlesbares Verzeichnis, fail-closed):** Given ein Verzeichnis unterhalb der Wurzel, das nicht gelesen werden kann, when der Lauf erfolgt, then Exit 2 — eine unvollständige Soll-Menge ist keine.
- **Negative (kollidierende Suchbegriffe, fail-closed):** Given eine Soll-Menge mit `x.md` **und** `x.md.bak`, when der Lauf erfolgt, then Exit 2 mit Nennung **beider** Mitglieder — `x.md` könnte nie als unerwähnt gemeldet werden, und ein grüner Lauf über dieser Menge wäre eine Deckungs-Aussage, die nicht gilt. Unter `basename` gilt dasselbe für denselben Dateinamen in zwei Verzeichnissen.
...
| 0.86.4 | 2026-09-06 | Zweite Review-Runde an der Implementierung: **die Härtung aus 0.86.3 kippte die Fehlerpolitik in die andere Richtung.** **(1) Die Grenz-Prüfung war zu weit und meldete ERWÄHNTE Artefakte als unerwähnt** — vier am Bestand gemessene Formen fielen durch: der Satz-Schlusspunkt (`siehe x.md.`), das deutsche Kompositum (`x.md-Datei`), die Kursivierung (`_x.md_`) und, unter dem **Default** `path`, **jede** `../`-relative Verlinkung. Letzteres ist das schwerste: An diesem Bestand kippten dadurch 21 von 84 korrekt verlinkten ADRs auf Befund, und ein Adopter mit dem üblichen Layout (Doku im Unterverzeichnis, relative Links) hätte seine **gesamte** Soll-Menge gemeldet bekommen. Die Grenze ist jetzt **asymmetrisch und eng**: links blockiert Buchstabe, Ziffer, `-` und `.`, rechts nur Buchstabe und Ziffer. **(2) Sie ist runen-basiert statt byte-basiert.** Die alte Fassung sah bei `Ätest.md` das zweite Byte des Umlauts und ließ durch — das stille Grün, gegen das die Härtung angetreten war, bestand bei jedem Nicht-ASCII-Nachbarn fort. **(3) Drei Grenzen sind der ausgewiesene Preis** und stehen jetzt in der Anforderung: ein Unterstrich-Kompositum, eine angehängte Endung und ein fremdes Pfad-Präfix decken das Mitglied darunter. Eine rein textuelle Regel kann Namensteil und Satzzeichen nicht trennen, wo dasselbe Zeichen beides ist; gewählt ist die Seite, die die häufigen Formen zählen lässt. **(4) Fünf Akzeptanzkriterien nachgetragen** — die drei Vertrags-Aussagen der Fassung 0.86.3 (eigenständige Nennung, `scan.ignore`-Pruning, unlesbares Verzeichnis) hatten keines, und die Liste stand unverändert bei zehn. **Nachgemessen und bestätigt:** die Erkennungsform-Aussage („zwölf Artefakte aus drei Repos, **einmal** abweichend") hält auch unter der neuen Semantik — nachgefahren mit dem ausgelieferten Modul statt mit `grep` | — |
| 0.86.3 | 2026-09-06 | Nachzug nach dem unabhängigen Review der **Implementierung** (drei HIGH, alle über Bruch-Tests belegt). **Die Anforderung ändert sich an einer Stelle wirklich: (1) „Vorkommen" heißt jetzt EIGENSTÄNDIGE NENNUNG, nicht beliebige Teilzeichenkette.** Die alte Fassung sagte „das wörtliche Vorkommen einer Zeichenkette" — damit deckte die Nennung von `image-test.md` das Mitglied `test.md` ab und ein Pfad mit fremdem Präfix das Mitglied darunter; ein Mitglied galt als erwähnt, das nirgends genannt war. Geprüft wird jetzt die linke und die rechte Grenze der Fundstelle, links unter `basename` mit Ausnahme des `/` (dort steht legitim ein Pfad-Präfix). **Das ist genau die Kollision, gegen die der Default `path` laut derselben Anforderung steht** — die Messung, die sie ausschließen sollte, prüfte Gleichheit von Basisnamen und nicht Enthaltensein. **(2) Ein Block ist EIN Paar, mit Preis.** Die Ist-Menge ist eine Vereinigung; zwei unabhängige Invarianten in einem Block halten **keine** von beiden. Gemessen: zwei Paare in einem Block meldeten „108 von 108" auch dann noch, als eine ADR vollständig aus ihrem Index entfernt war. Die Grenze steht jetzt in der Anforderung. **(3) Die zweite Scan-Achse ist benannt statt vorausgesetzt:** aufgesammelt wird aus dem ganzen Baum ab der Repository-Wurzel, unter der Skip-Liste **und** `scan.ignore`, aber **nicht** eingeschränkt auf `scan.roots`; ein unlesbares Verzeichnis ist Exit 2 statt stillem Schrumpfen. **(4) Die Erkennungsform-Wahl bekommt ihre Mess-Regel:** Wer `basename` wählt, prüft **zwei** Dinge — Eindeutigkeit der Basisnamen **und** dass keiner Endstück eines anderen ist. **(5) Die Zahlen sind neu gemessen und teils berichtigt:** am eigenen Bestand meldet `match: path` **84 von 84** ADRs und **24 von 24** Sensor-Dateien (nicht 23), ausnahmslos falsch; am Fremd-Bestand lieferten **beide** Formen dasselbe Ergebnis (4 Funde bei 5 Mitgliedern), weil jenes Handbuch mit vollem Pfad nennt — und eines der vier ist eine Test-Datei und damit kein Mangel. Damit ist der Kalibrierungs-Beleg am **fremden** Bestand erstmals mit dem Modul selbst gefahren; als Messung führte ihn die Zeile 0.86.1 bereits | — |
...
| 0.63.0 | 2026-08-22 | [`DC-FA-VER-001`](#dc-fa-ver-001--versions-pin-konsistenz-modul-versions-opt-in) um **mehrere Muster-Quellen-Paare** erweitert (`versions.patterns`, opt-in; Erweiterung statt neues Kürzel nach dem etablierten Schnitt-Kriterium — Einzelmodul-Frage ⇒ bestehende Anforderung ändern): ein Repo führt mehrere unabhängige Versions-Reihen, das Modul kannte bisher genau **eine**. Anlass ist die Beobachtung BEO-ALL/pin-bump-mirrors-ungated bei Zähler 3, deren benannte mechanische Form (Baseline-Tag in URLs und Prosa gegen den §Baseline-Pin, **zusätzlich** zum Image-Pin gegen das Release-Register) mit einem einzigen `pin-pattern` nicht baubar war. Jedes Paar trägt eigene Quelle und eigene Ausnahmen, die Selbst-Ausnahme der Quell-Datei ist **paar-lokal**; die Kurzform bleibt gültig und **ist** die einelementige Liste (ein Auswertungspfad), beide Schreibweisen zugleich sind fail-closed. Kein neuer Grund-Code, keine Änderung an der Befund-Form. Begründung in begleitender ADR | — |
