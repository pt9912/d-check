...
3. **Tabellen-Lexik.** Außerhalb von Fenced-Code beginnt eine Tabelle mit einer
   ...
   **Kommentar-Suffix.** Ein **abschließender** HTML-Kommentar, der die
   **ganze letzte Zelle** ausmacht (`| a | b | <!-- … -->`, mit oder ohne
   nachfolgende Pipe), ist **keine Zelle**, sondern ein Zeilen-Suffix und wird
   vor dem Zellen-Zählen entfernt — genau einer, nur am Zeilenende. So trägt eine
   Tabellenzeile d-checks eigene Direktiven-Konvention
   (`<!-- d-check:ignore (…) -->`), die außerhalb einer Zelle stehen **muss**:
   in einer Zelle wäre sie Inhalt. Ein Kommentar **innerhalb** einer Zelle oder
   in der Zeilenmitte bleibt unverändert Zellinhalt.
