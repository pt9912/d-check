...
[`modul-03-spec.md` §Ziel-Form: Spezifikation](../.harness/baseline/v6.9.0/regelwerk/modul-03-spec.md#ziel-form-spezifikation).
...
5. Dreizehn `.PHONY`-Targets, jeweils mit `##`-Annotation (die das `help` des
   ...
   - `doc-targets` (`--enable targets` + auf `targets` fokussierte `--disable`-Liste,
     ...
     **kein** `--range`/`--staged` (hermetisch — nur der Arbeitsbaum: Makefile +
     ...
     Skript-Kopie,
   - `doc-structure` (`--enable structure` + auf `structure` fokussierte `--disable`-Liste,
     [`DC-FA-STRUCT-001`](lastenheft.md#dc-fa-struct-001--struktur-invarianten-innerhalb-eines-dokuments-modul-structure-opt-in));
     **kein** `--range`/`--staged` (hermetisch — nur der Arbeitsbaum: die Dokumente
     selbst). Die `--disable`-Liste analog aus `ValidModules` ohne `structure`
     abgeleitet; verteilt die Prüfung der Struktur-Invarianten an Konsumenten ohne
     Skript-Kopie,
   - `doc-usage` (`--help`,
     [`DC-FA-CLI-001`](lastenheft.md#dc-fa-cli-001--aufruf-und-scan-wurzel)) — das
     Target exponiert einen **Modus**, kein Regelmodul, und trägt deshalb keine
     `--enable`/`--disable`-Wahl — darin steht es neben `doc-check`,
     `doc-trace`, `doc-complete`, `doc-doctor`, `doc-repair` und `doc-help`
     (gezählt am erzeugten Fragment: sechs der dreizehn tragen eine
     Fokus-Liste, sieben nicht). Das Recipe ist
     mit `@` **echo-unterdrückt** (wie `doc-repair`/`doc-help`): die Hilfe ist
     die Nutzlast, nicht ein Befund. **Sie erscheint auf `stderr`** — der
     Hilfe-Pfad setzt die Ausgabe des Flag-Satzes dorthin, bevor er sie
     schreibt; eine Umleitung von stdout fängt nichts. Der Mount bleibt, obwohl
     `--help` ihn nicht braucht (Kurzschluss vor der Wurzel-Auflösung): die
     Form ist für alle Targets dieselbe zugesagt,
...
3. **Existenz-Auflösung.** Eine Kennung gilt als **definiert**, wenn sie in der
   ...
   (einmaliges Lesen).
4. **Validierung beim Lauf-Start.** Jede `defined-in`-Quelle muss existieren,
   innerhalb der Repo-Wurzel liegen und eine **Datei** sein (sonst Exit 2 ohne
   Prüfung). Das Verzeichnis ist hier — anders als bei einem `ids`-Target —
   **kein** zulässiger Wert: das Modul liest die Quelle, ein Verzeichnis
   lieferte die leere Definitionsmenge und damit jede Kennung als undefiniert.
   Die Gültigkeit der `exempt-paths`-Globs prüft dagegen der **Config-Rand**
   (Decode-Zeit, segmentweise wie die Laufzeit matcht; leerer Eintrag ⇒ Exit
   2). **Link-Policy gilt nicht** (in Fences kein Markdown-Link
   möglich). Ohne `diagrams`-Block ist der Befundsatz byte-identisch
   ([`DC-QA-02`](lastenheft.md#dc-qa-02--determinismus)).
5. **Ventile.** Wie `ids`/`codepaths`/`versions`: die Glob-Liste `diagrams.exempt-paths`
   ...
   **festgelegt, nicht abgeleitet**:
   ...
   - Er wirkt an **zwei Orten**: auf einer Diagramm-Zeile für **diese** Zeile,
     ...
     Marker dort ist ausschließlich Ventil. Die **schließende** Fence-Zeile
     ist **kein** Ventil-Ort — dort bleibt der Marker folgenlos.
   - Beide Ventile wirken **scan-seitig**. Die Definitionsmenge aus Schritt 3
     kennt sie nicht: eine im `defined-in` als illustrativ markierte Kennung
     definiert weiter. Die Ventile unterdrücken Befunde, sie ändern nicht die
     Wahrheit über die Definitionsmenge.
...
2. **Geschützte Kandidaten.** Die Klasse `vcs.paths` (Glob, `matchGlob` wie
   ...
   **„Nicht lesbar" heißt seit slice-226 <!-- d-check:status-provenance -->
   nicht mehr „unter einem anderen Namen als erwartet".** Der VCS-Port löst
   einen Pack, dessen Datei nicht unter dem git-kanonischen Präfix `pack-`
   liegt (z. B. `loose-<hash>.pack`, wie `git maintenance
   run --task=loose-objects` es schreibt), zusätzlich unter seinem
   kanonischen Namen auf, sofern die Datei einen gültigen SHA1/SHA256-Hash
   als Namens-Suffix trägt **und** eine passende Index-Datei existiert — ohne
   das gelesene Repository zu verändern. Ein Pack ohne ein solches Suffix
   oder ohne passenden Index bleibt unsichtbar, und die Enumeration bricht
   fail-closed ab, wenn er die einzige Quelle eines benötigten Objekts ist —
   dieselbe Zusage wie zuvor, nur nicht mehr an einen einzigen Dateinamen
   gebunden.
...
3. **Abschnitte finden.** Gesucht werden alle Zeilen, die (a) **außerhalb** eines
   ...
   **Abschnitts-Ausnahme, vor der Kardinalität.** Ist `exempt-section-pattern`
   ...
   Schritt 2, eine Granularitätsstufe tiefer. Dieser Befund trägt die
   **modul-eigene** Meldung auch dann, wenn die Regel einen `hint` führt: er
   gehört zur Klasse *„die Regel hat nicht gemessen"*, die vom verfassten
   Hinweis ausgenommen ist.
   **Ist `exempt-expect-count` gesetzt, tritt an die Stelle dieser Härte eine
   Zählung:** weicht die Zahl der tatsächlich abgezogenen Abschnitte von der
   deklarierten ab — in **beide** Richtungen und **unabhängig** davon, ob eine
   Restmenge bleibt —, entsteht `section-exempt-mismatch` (`line` = 1);
   stimmt sie, entsteht **kein** Befund, auch bei geleerter Menge. Dieser
   Befund ist **nicht** von `hint` ausgenommen: die Regel hat gemessen und
   eine Deklaration widerlegt. Der Abzug läuft **vor**
   Schritt 4, weil er die **Grundmenge** erklärt — was ausgenommen ist, kann
   `sections: one` nicht mehrdeutig machen. Ein **gesetztes**
   `exempt-section-pattern` geht in die **Regel-Identität** ein (`… :: ohne
   <muster>`): zwei Regeln über denselben Selektor, von denen eine einen
   Bestand ausnimmt, sind verschiedene Zusagen; ein **leeres** geht nicht ein,
   sonst änderte sich das `target` jeder Bestandsregel.
...
6. **Bedingungen prüfen**, jede optional, jede mit **eigenem** Grund-Code, damit
   ...
   | `open-tasks-require-marker` | **nur** wirksam, wenn `max-open-tasks` bereits Überschuss-Items meldet (sonst greift die Bedingung nicht — der Normalfall bleibt unberührt): die geforderte Marke (`hasMarker`-Form wie bei `require-all`) ist im durchsuchten Text vorhanden — **ohne** `open-tasks-require-marker-section` der bereinigte Text **desselben** Abschnitts, **mit** ihr die Vereinigung aller Abschnitte, deren rohe Überschriften-Zeile matcht (RE2, samt `#`-Folge, wie `section-pattern`) — dann entfallen **alle** `section-tasks-open`-Einzelbefunde dieses Abschnitts | fehlt sie (oder trifft `open-tasks-require-marker-section` keinen Abschnitt) ⇒ **ein** `section-open-tasks-marker-missing` **statt** der Einzelbefunde, `line` = Überschriftszeile |
   ...
   **nicht** mit. Der Vergleichsgegenstand ist der **getrimmte Item-Text hinter
   ...
   den Schlüssel bleibt die Meldung **unverändert**. Die Sichtbarkeit hat
   **zwei** Grenzen: sie greift, **solange die Regel meldet** (wer so breit
   ignoriert, dass die Schwelle nie fällt, sieht nichts), und ein **`hint`**
   derselben Regel **ersetzt** die Meldung samt der Zahl — er gewinnt gegen
   jede modul-eigene Meldung einer verletzten Bedingung.
   ...
   **Zellenlänge (`table.column`)** — die **zweite**
   ...
   - **Spalte binden, je Tabelle.** Die Datenzeilen-Bestimmung ist dieselbe wie
     ...
     **kein** Treffer macht die Tabelle für diese Bedingung irrelevant (kein
     ...
     ab); **mehr als einer** ⇒ `section-column-missing` auf der **Kopfzeile**,
     denn welche der beiden Spalten gemeint ist, sagt die Konfiguration nicht.
     Eine Tabelle **ohne** Kopf-/Trennzeile bindet nichts.
   - **Messen.** Je Datenzeile einer bindenden Tabelle: die Zelle an der
     gebundenen Position, **getrimmt**, gezählt in **Zeichen** (Unicode-Runen,
     nicht Bytes). Weniger als `cell-min-chars` ⇒ `section-cell-undersized`,
     mehr als `cell-max-chars` ⇒ `section-cell-oversized`, jeweils an
     **dieser** Zeile — dort ist die Reparatur. Die Untergrenze wird **zuerst**
     geprüft; beide zugleich sind am Config-Rand ausgeschlossen. Eine
     Datenzeile mit **zu wenigen Zellen** ⇒ `section-column-missing` an dieser
     Zeile: Befund statt stillem Übersprung, sonst schaltete eine kaputte Zeile
     die Messung wortlos ab.
   - **Eine Obergrenze allein lässt die leere Zelle durch** — 0 Zeichen liegen
     unter jeder Schwelle. Wer zusagt, dass eine Spalte *gefüllt* ist, sagt das
     mit `cell-min-chars`; die beiden Grenzen tragen **eigene** Grund-Codes,
     weil die Reparatur eine andere ist (ausfüllen statt kürzen).
   ...
   - **Leerlauf.** Bindet **keine** Tabelle des Abschnitts die Spalte ⇒
     `section-column-missing` an der Abschnitts-Überschrift. Die Bedingung zu
     setzen **ist** die Behauptung, dass es diese Spalte gibt — dieselbe
     Doppel-Rolle wie beim Chronologie-Leerlauf.
   - **Grenze, benannt statt zugesagt:** gemessen wird die Zelle, **wie sie
     dasteht** — Markdown-Syntax eingeschlossen. Eine Zelle, die aus einem
...
| `matrix.classes[].paths` | string[] | leer | Glob (Mitgliedschaft). **Leer zulässig:** eine Klasse ohne `paths` ist ein **Token-Ziel** — sie hat keine Mitglieder und dient ausschließlich als **Ziel** einer Token-Referenz ([`DC-FA-MTX-003`](lastenheft.md#dc-fa-mtx-003--token-basierte-referenz-richtung-mit-provenance-marker-modul-matrix)) — ihr Gegenstand ist eine Zeichenkette, kein Dokument. Ohne `paths` **und** ohne `token` ist die Klasse inert |
...
| `diagrams.fences` | string[] | `[mermaid]` — **auch bei explizit leerer Liste** (Abweichung von der Präambel: `fences: []` schaltet das Modul nicht ab, sondern lässt den Default gelten) | zu öffnende Fence-Sprachen (Info-String, kleingeschrieben verglichen); leerer **Eintrag** ⇒ Exit 2. **Nur** diese Fences werden geöffnet |
...
| `diagrams.patterns[].defined-in` | string | — | Pflicht; **Datei** (kein Verzeichnis — Abgrenzung zu `ids.patterns[].target`), in der die Kennung als Token **außerhalb von Fences** vorkommen muss. Muss existieren, innerhalb der Repo-Wurzel liegen und eine Datei sein (sonst Exit 2 beim Lauf-Start) |
| `diagrams.exempt-paths` | string[] | leer | Glob (wie `scan.ignore`, relativ zur Repo-Wurzel); Dateien ganz ohne `diagrams`-Prüfung — datei-weit. **Segmentweise** validiert (so matcht die Laufzeit); leerer Eintrag oder ungültiges Segment ⇒ Exit 2 |
...
| `planning.waves.mode` | string | `one` | Kardinalitäts-Modell der ersten Wellen-Aussage: `one` = Singleton (`hasActive` ⇔ genau eine Wellen-Kennung unter den flachen Dokumenten), `many` = Kennungs-Mengen-Gleichheit `planning.heading`-Block ⇔ Kennungs-Menge der flachen Dokumente (beide Richtungen, jede Kardinalität; der Ruhe-Marker geht **nicht** ein). Andere Werte — auch der **explizit leere** String — ⇒ Exit 2 |
...
| `structure[].open-tasks-require-marker` | string | leer (aus) | benannte Marke (dieselbe `hasMarker`-Form wie `require-all`), **konditioniert** auf `max-open-tasks`: greift **nur**, wenn dessen Zählung für den Abschnitt bereits mindestens einen Überschuss-Fund liefert — im Normalfall (alle Haken gesetzt) bleibt sie wirkungslos. Ist die Marke im durchsuchten Text vorhanden, entfallen **alle** `section-tasks-open`-Einzelbefunde dieses Abschnitts (Erlaubnis); fehlt sie, ersetzt **ein** `section-open-tasks-marker-missing` (`line` = Überschriftszeile) dieselben Einzelbefunde (Pflicht) — nie beide Grund-Codes zugleich für denselben Abschnitt. **Ohne** `max-open-tasks` ⇒ Exit 2 (halbe Aktivierung, wie `tasks-ignore-pattern` ohne `max-tasks`) |
| `structure[].open-tasks-require-marker-section` | string | leer (derselbe Abschnitt) | RE2 gegen **dieselbe** rohe, getrimmte Überschriften-Zeile wie `section-pattern` (samt `#`-Folge) — verlegt die Marken-Suche von `open-tasks-require-marker` in **jeden** Abschnitt der Datei, dessen Überschrift trifft (Vereinigung mehrerer Treffer). **Abwesend** sucht die Marke weiterhin im **gezählten** Abschnitt selbst (byte-identisch zum Vorzustand ohne den Schlüssel). Trifft das Muster **keinen** Abschnitt, gilt die Marke als fehlend — eine Datei ohne den benannten Abschnitt hat per Definition auch keine Marke darin. **Ohne** `open-tasks-require-marker` ⇒ Exit 2 (halbe Aktivierung); nicht kompilierend ⇒ Exit 2 |
...
| `SPEC-083` | `section-open-tasks-marker-missing` | structure | `open-tasks-require-marker` ist gesetzt, `max-open-tasks` meldet für den Abschnitt bereits mindestens einen Überschuss-Fund, und die geforderte Marke fehlt im bereinigten Text — **ein** Befund für den ganzen Abschnitt (`line` = Überschriftszeile) **statt** der sonst je Item gemeldeten `section-tasks-open`. Eigener Code, weil die Reparatur eine andere ist: dort Haken setzen oder Punkt auflösen, hier die Marke ergänzen |
