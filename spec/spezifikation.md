...
| `matrix.classes[].paths` | string[] | leer | Glob (Mitgliedschaft). **Leer zulässig:** eine solche **Token-Ziel-Klasse** hat keine Mitglieder und dient ausschließlich als **Ziel** einer Token-Referenz ([`DC-FA-MTX-003`](lastenheft.md#dc-fa-mtx-003--token-basierte-referenz-richtung-mit-provenance-marker-modul-matrix)) — ihr Gegenstand ist eine Zeichenkette, kein Dokument. Ohne `paths` **und** ohne `token` ist die Klasse inert |
