...
6. **Bedingungen prüfen**, jede optional, jede mit **eigenem** Grund-Code, damit
   ...
   | `open-tasks-require-marker` | **nur** wirksam, wenn `max-open-tasks` bereits Überschuss-Items meldet (sonst greift die Bedingung nicht — der Normalfall bleibt unberührt): die geforderte Marke (`hasMarker`-Form wie bei `require-all`) ist im bereinigten Abschnitts-Text vorhanden — dann entfallen **alle** `section-tasks-open`-Einzelbefunde dieses Abschnitts | fehlt sie ⇒ **ein** `section-open-tasks-marker-missing` **statt** der Einzelbefunde, `line` = Überschriftszeile |
...
| `structure[].open-tasks-require-marker` | string | leer (aus) | benannte Marke (dieselbe `hasMarker`-Form wie `require-all`), **konditioniert** auf `max-open-tasks`: greift **nur**, wenn dessen Zählung für den Abschnitt bereits mindestens einen Überschuss-Fund liefert — im Normalfall (alle Haken gesetzt) bleibt sie wirkungslos. Ist die Marke im **bereinigten** Abschnitts-Text vorhanden, entfallen **alle** `section-tasks-open`-Einzelbefunde dieses Abschnitts (Erlaubnis); fehlt sie, ersetzt **ein** `section-open-tasks-marker-missing` (`line` = Überschriftszeile) dieselben Einzelbefunde (Pflicht) — nie beide Grund-Codes zugleich für denselben Abschnitt. **Ohne** `max-open-tasks` ⇒ Exit 2 (halbe Aktivierung, wie `tasks-ignore-pattern` ohne `max-tasks`) |
