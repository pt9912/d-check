...
| 2026-07-03 | Nachtrag nach Closure (Auftraggeber-Einwand: ein done/-Slice ist kein lebender Träger für offene Verpflichtungen): das `net/*`-Rest-Delta ist **getilgt** statt getragen — die Enumeration in der `.a-check.yml` weicht einer RE2-**Ausschluss-Klasse** (alles nach `net/` außer exakt `url`, ohne Lookahead); neue stdlib-Subpakete fallen automatisch unter den R1-Fang. Proben-belegt (`net/futurepkg`/`net/urlx` im Kern rot, `net/url` grün). Config-only, keine Lockerung. |
