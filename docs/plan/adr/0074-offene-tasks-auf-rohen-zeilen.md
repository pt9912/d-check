...
| 2026-08-30 | **Drei Zahlen der §Fitness Function stimmen nicht mehr, und eine Zusage daneben war zu weit.** Es sind **zehn** Kern-Tests, nicht sieben — dazugekommen sind die Abschnittsgrenze (beidseitig scharf, inklusive der letzten Abschnitts- und der letzten Dateizeile), die Inline-Code-Grenze in beiden Richtungen und die Lexik-Kopplung. Letztere ist die lehrreiche: ihre **erste** Fassung war tautologisch — sie prüfte *„`offenerHaken` trifft nur, was `taskItemRE` trifft"*, und weil `offenerHaken` genau dessen Treffer liest, ist die Bedingung per Konstruktion unerfüllbar. Gemessen: ein zweites, gedoppeltes RE2 daneben — exakt die Drift, gegen die der Test namentlich stand — ließ ihn **grün**. Die jetzige Fassung **erweitert** `taskItemRE` um eine Form, die es heute nicht gibt, und verlangt, dass `offenerHaken` mitzieht; sie fängt die Doppelung. Klasse [`BEO-023`](../planning/observations.md), gefunden vom Review |
| 2026-08-30 | **Die Byte-Identitäts-Zahl nennt jetzt ihren Korpus, und sie ist ohne ihn nicht reproduzierbar.** Der Feat-Commit sagt *„276 Befunde"* ohne Glob und Selektor; die Verifikation hat mit drei plausiblen Varianten 164, 331 und 161 gemessen. Der Lauf ist: Glob `docs/plan/planning/**/slice-*.md`, Selektor `^#{2,3} [0-9]+\. Definition of Done`, `sections: each`, `max-tasks: 3`, nur `--enable structure`. Die **tragende** Aussage — `diff` leer zwischen Vorgänger- und Nachfolger-Image — hat die Verifikation unabhängig mit einem selbst gebauten Vorgänger-Image belegt |
| 2026-08-30 | **Die Fence-Divergenz-Messung ist eine Eigenschaft ihres Verfahrens, und das gehört dazugesagt.** Der Vergleich der beiden Fence-Automaten über den Bestand hängt daran, ob die **Infozeilen-Regel** (eine Backtick-Fence, deren Infozeile einen Backtick enthält, ist kein Öffner) auf beiden Seiten angewandt wird. Wendet man sie nur auf den Toggle an, bleibt **ein** Treffer — die Prosa von [ADR-0042](0042-markdown-lexik-folgt-commonmark.md) selbst; wendet man sie beidseits an, bleibt **null**. Beide Fassungen tragen dieselbe Aussage (kein Realfall, der ein Verhalten änderte), aber die Zahl gehört nicht ohne ihr Verfahren zitiert |
