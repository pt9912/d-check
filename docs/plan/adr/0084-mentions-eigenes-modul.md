...
| 2026-09-06 | **Berichtigung §Kontext (unabhängiger Review, M-1).** Der Satz *„Die Konvention „das Ist-Dokument zitiert Kennungen" existiert in diesem Repo nicht, also kann `trace.coverage` die Achse prinzipiell nicht bedienen"* ist gegen **ein** Dokument gemessen (`benutzerhandbuch.md`, 5 Nennungen) — während dieselbe Entscheidung zwei Absätze weiter das Messen gegen ein Dokument zum Fehler erklärt. Über die **Ist-Menge** `docs/user/` sind es **35 Vorkommen** und **24 verschiedene** `DC-*`-Kennungen bei 52 Anforderungen (`operations.md` allein 24). Die Konvention existiert also **teilweise**, und die Prämisse trägt in dieser Form nicht. **Der Entscheid steht davon unberührt**, weil er auf dem anderen Bein steht: `trace.coverage` misst über **Kennungen** und ist damit schema-gebunden, diese Achse über **Pfade**. Was fällt, ist ein Zusatz-Argument, nicht der Grund. |
| 2026-09-06 | **Berichtigung §Verglichene Alternativen (unabhängiger Review, M-2).** Die Zeile *„an sieben Artefakten aus zwei Repos lieferten beide Formen **dasselbe** Ergebnis"* nennt ihre Menge nicht und ist falsch. Nachgemessen über **drei** benannte Mengen — d-check `tools/*.sh` (5), `ai-harness-init` `harness/*.md` (2), `u-boot` `scripts/*.sh` + `tools/*.py` (5), zusammen **zwölf** Artefakte aus **drei** Repos — weichen Pfad- und Dateinamen-Form **einmal** ab: `harness/README.md` ist per Pfad **nicht** erwähnt, per Dateiname **doch**. Die Nennung im dortigen Handbuch meint die **Projekt-Wurzel**-`README.md`, nicht das Artefakt der Soll-Menge. Das ist ein Falsch-Positiv der laxen Form — und stützt den Default `path` **stärker** als die vorige Fassung: nicht „kein Unterschied", sondern „der einzige gemessene Unterschied ist ein Fehlalarm". |
| 2026-09-06 | **Berichtigung §Konsequenzen (unabhängiger Review, M-7).** „Ein **22.** Regelmodul" ist die falsche Ordinalzahl: `validModules()` führt **22** Namen **ohne** `mentions`; es wäre das **23.**. Die Zahl steht ausgerechnet in dem Punkt, der von den Spiegeln handelt, die eine Modul-Aufnahme still mitändert. |
| 2026-09-06 | **Berichtigung §Kontext, Inventur (unabhängiger Review, M-8).** Die Zählung „zehn Repos … plus zwei ohne erkennbares Soll-Schema" hält der Nachmessung nicht stand. Reproduzierbar über `grep -cE '^#{3,4} [A-Z][A-Z0-9]*-' spec/lastenheft.md` bei vorhandenem `docs/user/`: **zwölf** Repos führen beide Seiten — a-check 21, b-cad 53 (nicht 23), b-trace 0, c-hsm-doc 151 (nicht 89), cmake-xray 0, d-check 52, k-deskflight 185, m-trace 4 (nicht 7), u-boot 139, ai-harness-init 14, bedrock-eu-guard 114, belief-agent 56 (nicht 15). Die Abweichungen entstehen daran, dass die erste Zählung nur `###` sah, wo mehrere Repos ihre Anforderungen auf `####` führen. **Die Schemata-Aussage stimmt in der Zahl und war in der Liste falsch:** heading-seitig sind es `LH`, `HSM`, `DC`, `AC` und die repo-lokale Gruppe `CLI`/`CORS`/`MVP` — **fünf**; das in der ersten Fassung genannte `S-*` kommt in **keiner** der zwölf Soll-Seiten vor. Im Rumpf sind es deutlich mehr (`OPD`, `PH`, `SA`, `TC`, `API`, `OPS`, `GG`, `E`, `F`, `IRS`, `Q`, `T`, `TEST`), und auch `b-trace` und `cmake-xray` führen dort Kennungen — „ohne erkennbares Soll-Schema" gilt nur für die Überschriften-Ebene. **Die Richtung des Arguments überlebt und wird stärker** (mehr Heterogenität, nicht weniger); die Ziffern der ersten Fassung überleben nicht. |
| 2026-09-06 | **Berichtigung §Entscheidung (a), Grund 1 (unabhängiger Review, M-9).** *„XREFs eigene Zusagen (Vakuum-Prüfung, Namensraum-Vorbedingung) sind über diesen Schlüssel definiert"* trägt nur zur Hälfte. Die **Vakuum**-Prüfung ist über `R` definiert (sie misst kantenleere Sichten, und eine Kante ist ein `R`→Artefakt-Paar). Die **Namensraum-Vorbedingung** ist im Quelltext dagegen über die **Artefakt**-Seite gefasst — *„Vorwärts- und Rückwärts-**Artefakt-IDs** werden mit demselben `design-pattern` erkannt"* —, und `R` kommt darin nicht vor. Grund 1 trägt damit über die Vakuum-Hälfte und über die Struktur (`F(R)`, `B(R)`, `keys(F) ∪ keys(B)`, die drei `req-pattern`), nicht über die Namensraum-Hälfte. Re-Evaluierungs-Trigger 2 bleibt gültig, stützt sich aber auf die erste Hälfte. |
| 2026-09-06 | **Form-Abweichung benannt (unabhängiger Review, L-2, L-5).** §Verglichene Alternativen führt die Spalten `Alternative` und `Verworfen, weil` und damit nur die Contra-Hälfte; Vorlage und `modul-04-adrs.md` verlangen „mindestens drei Alternativen, **jede mit Trade-off**" und „**nichts tun**" als eine davon — beides fehlt, gefolgt ist dem Nachbarn ADR-0083 statt der Vorlage. Der Bestand ist damit **nicht** entlastend: Form-Fragen sind in der Vorlage nachzuschlagen, nicht am Nachbar-Artefakt abzulesen. Zwei Verwerfungs-Zeilen berufen sich außerdem auf eine „`Accepted`-Anforderung" — `spec/lastenheft.md` führt `Status: Draft` und kennt kein Status-Feld je Anforderung; `Accepted` ist ADR-Vokabular. Die Verwerfungen tragen über §3 des Slice-Plans (Out-of-Scope), nicht aus dem genannten Grund. |
| 2026-09-06 | **Zweite Berichtigung der Inventur — die erste war falsch (Review-Runde 2, R2-M-1).** Die in der Zeile darüber genannte Methode `grep -cE '^#{3,4} [A-Z][A-Z0-9]*-'` zählt keine Kennungen, sondern **jede Überschrift, die mit Großbuchstaben und einem Bindestrich beginnt** — bei `m-trace` waren das `MVP-Abgrenzung`, `CLI-Ziel`, `MVP-Entscheidung` und `CORS- und CSP-Grundregeln`, also deutsche Komposita. Genau daraus entstand die behauptete „repo-lokale Gruppe `CLI`/`CORS`/`MVP`" als **fünftes** ID-Schema: eine Kennungs-Familie, die es nicht gibt. Mit einer Methode, die verlangt, was eine Kennung ausmacht — Präfix **plus Ziffernblock**, `grep -cE '^#{2,4} [A-Z][A-Z0-9]*(-[A-Z0-9]+)*-[0-9]+'` — ergibt sich: **zwölf** Repos führen beide Seiten, davon **neun** mit kennungstragenden Überschriften und **drei** ohne (`b-trace`, `cmake-xray`, `m-trace`); die Schemata sind **vier**, nicht fünf: `LH` (6 Repos), `HSM`, `DC`, `AC`. `u-boot` zählt 137, nicht 139 — die alte Methode zählte dort zwei Überschriften doppelt. **Der Fehler ist schlimmer als der, den er beheben sollte:** die erste Fassung hatte nur falsche Zahlen, diese hier hat eine Kategorie erfunden. **Die Richtung des Arguments überlebt auch das** — vier verschiedene Schemata über neun Repos, plus drei Repos ohne jede Heading-Kennung, sind für eine kennungsbasierte Lösung genau das Problem, das die pfadbasierte nicht hat. |
| 2026-09-06 | **Berichtigung einer Zahl in der M-1-Zeile (Review-Runde 2, R2-M-4).** Dort steht „`operations.md` allein 24". Gemessen sind es **26 Vorkommen** und **21 verschiedene** Kennungen; die 24 ist die Zahl der **verschiedenen** Kennungen über die **ganze** Ist-Menge und war der falschen Datei zugeordnet. |
| 2026-09-06 | **Angleichung an das Lastenheft (Review-Runde 2, R2-M-7).** §Was aus diesen beiden folgt führt die Messung „die 22 Regelmodule gegen `docs/user/` → null Funde" weiter als eigene Gegenprobe, während [`DC-FA-MENT-001`](../../../spec/lastenheft.md#dc-fa-ment-001--erwähnungs-deckung-einer-artefakt-menge-modul-mentions-opt-in) sie seit 0.86.1 ausdrücklich als **Kalibrierungs-Messung ohne Beleg-Kraft** führt — sie benutzt eine Literal-Menge, die Out-of-Scope (2) ausschließt. Es gilt die Fassung des Lastenhefts; die ADR-Stelle ist damit überholt, nicht widersprechend. |
