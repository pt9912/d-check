...
| 2026-07-01 | Skript-Mechanik (`tools/trace-check.sh`) teil-superseded durch [ADR-0027](0027-commits-traceability-modul.md) (slice-056): das Gate `make trace-check` läuft auf das d-check-Modul `commits` um (reine-Go-VCS-Port, im Image verteilt statt kopiertem Skript); `tools/trace-check.sh` ist mit dieser Ablösung per `git rm` entfernt, seine immutable Inline-Referenz als `codepaths.ignore-refs`-Tombstone deklariert. **Policy, Bindepunkt und CI-Topologie unverändert** (Traceability-Regel; `commit-msg`-Hook + PR-/Push-CI, beide Quadranten). |
| 2026-09-04 | Der in der Zeile vom 2026-06-21 als Klartext-Pfad genannte Review-Report ist archiviert (slice-199, `tools/archive-wave -review=<datei>`, [`MR-063`](../../../harness/conventions.md#mr-063)): Stub unter `docs/reviews/archiv/2026-06-21-adr-0013-pr-ci-traceability.md`, Volltext im dortigen `-archiv.zip`. Die Klartext-Zeile selbst bleibt unverändert (§3.5 Kern-Schutz) und benennt den Pfad zum Zeitpunkt der Einarbeitung. |
