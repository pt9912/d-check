...
| 2026-06-29 | Skript-Mechanik (`tools/completeness-check.sh`) teil-superseded durch [ADR-0026](0026-completeness-in-product-gate.md) (slice-055): `make completeness-check` läuft auf den in-Produkt-Flag `--trace --require-complete` ([`DC-FA-CLI-011`](../../../spec/lastenheft.md#dc-fa-cli-011--vollständigkeits-prüfung-als-opt-in-exit-code)) um, das Skript ist entfernt + als `codepaths.ignore-refs`-Tombstone deklariert. **Policy und Bindepunkt unverändert** (Waise → FAIL, in `fullbuild`, nicht `gates`/`ci`); Core dieser ADR unangetastet. |
| 2026-06-23 | Provenienz (slice-044): neue Anforderung [`DC-FA-CLI-011`](../../../spec/lastenheft.md#dc-fa-cli-011--vollständigkeits-prüfung-als-opt-in-exit-code) ergänzt einen **opt-in** Produkt-Strict-Exit (`--trace --require-complete`, Waise ⇒ Exit 1) für Konsumenten-Makefiles (`doc-complete`-Fragment-Target). Der **Default**-`--trace` bleibt advisory (Exit 0); die hier gewählte Wrapper-Mechanik und Entscheidung bleiben unberührt (kein Supersede). |
