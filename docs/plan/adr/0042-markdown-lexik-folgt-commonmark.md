...
| 2026-08-30 | **Offener Punkt (a) erneut gemessen — weiterhin unbelegt.** Der Text sagt, `proseLines` sei ein *„naiver Toggle"* neben einem zweiten Automaten, der Zeichen und Länge prüft, und beide Punkte bekämen einen Sensor erst, *„wenn einer existiert"*. Für [ADR-0076](0076-spans-am-closure-bindepunkt.md) sind beide Lesarten des Produkts — `FenceToggle` gegen `FenceRun`+`FenceCloses` — über **alle 676** Markdown-Dateien dieses Repos gegeneinander gefahren: **null Abweichungen**. Die Messung kann finden: drei konstruierte Fälle (Infostring hinter dem Schluss-Fence, Zeichenwechsel `` ``` ``→`~~~`, zu kurzer Schluss-Run) meldet sie, die saubere Datei nicht. Der Trigger ist damit **nicht** eingetreten, und der Punkt bleibt offen — bei 676 Dateien statt bei 522. **Was sich stattdessen geändert hat:** der Closure-Bindepunkt fährt seit [ADR-0076](0076-spans-am-closure-bindepunkt.md) `spans` mit, womit der **unbeendete** Fence dort laut wird, ohne dass die Lexik angefasst werden musste |
