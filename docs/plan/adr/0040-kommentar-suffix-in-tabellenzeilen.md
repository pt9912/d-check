...
**Status:** Accepted — **wieder aufgenommen und umgesetzt 2026-07-18, sicher auf
...
| Datum | Ereignis |
|---|---|
| 2026-07-18 | **Accepted.** slice-074 umgesetzt (die Ein-Kommentar-Zellen-Nachsicht in `consumeTableRows`), reviewt (R4 ACCEPT-WITH-NITS, 0 HIGH/MEDIUM; R4-F-1 den R3-F-1-Kombinationsfall hart gebunden, R4-F-2 `htmlCommentCell` auf einen ganzzelligen Kommentar verengt) und als **v0.48.1** (SemVer-Patch, rot→grün) freigegeben. Realdatenbeleg: grid-gyms `architecture.md:913` läuft durch — entblockt slice-071. |
