...
| 2026-06-29 | Die „Skript bleibt pfad-stabil"-Teilentscheidung superseded durch [ADR-0025](0025-codepaths-ignore-refs.md) (slice-054): `tools/adr-immutable-check.sh` ist per `git rm` entfernt, seine immutablen Inline-Referenzen als `codepaths.ignore-refs`-Tombstones deklariert. **VCS-Port, Modul `vcs` und der Dogfood-Ersatz des `adr-check`-Gates bleiben unverändert gültig** (Teil-Supersede; Core dieser ADR unangetastet). |
