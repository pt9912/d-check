...
| 2026-09-03 | **Re-Evaluierungs-Trigger „Aufnahme in `gates`" eingetreten** — welle-86s eigene Trigger-Prüfung (vier reale `git commit`-Proben) maß, dass ein fehlender Review-Report weder vom lokalen Übergangs-Wächter noch von `make gates` abgewiesen wurde. Genau der hier benannte zweite Trigger („sobald ein voller Wellen-Zyklus mit `review-coverage` grün gelaufen ist"): welle-86 ist dieser Zyklus. Eingelöst durch [ADR-0082](0082-uebergangswaechter-reviews-observations.md) — `reviews` ist jetzt Teil von `make verify-closure-notes` (dem Übergangs-Wächter aus slice-175), weiterhin bewusst **nicht** Teil der unconditional `gates`-Zusammensetzung selbst. |
