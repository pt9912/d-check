...
|---|---|---|
| 2026-08-28 | Accepted — die Traceability-Kollision war vor der Konfiguration gemessen und ohne Gate-Lockerung gelöst | [slice-167](../planning/done/wellenlos/slice-167-dependabot.md) |
| 2026-08-28 | **Die Begründung des `docker`-Ausschlusses war falsch — beide Sätze, per Geschichte-Anhang zurückgenommen (der Kern bleibt immutabel).** Entscheidung 4 sagt, ein Dependabot-PR *„höbe den Tag und ließe den Digest stehen"* und Dependabot stelle die Frage *„anderer Bau desselben Tags"* gar nicht. **Gegen `dependabot-core` nachgemessen ist beides falsch:** der Docker-Updater hebt Tag **und** Digest gemeinsam, und er stellt die Digest-Frage ausdrücklich — für einen nicht-vergleichbaren Tag wie `nonroot` sogar ohne Unterdrückung. **Der Ausschluss selbst steht**, aber auf [ADR-0011](0011-digest-pins-build-gate-images.md) §Entscheidung 4: ein Digest-Hoist ist ein **bewusster Commit**, kein Auto-Update. Das ist eine Policy, kein technischer Bruch. **Dritte, unabhängige Tatsache, die hier fehlte:** `FROM golang:${GO_VERSION}@sha256:…` parst Dependabot ohnehin nicht — sein Tag-Muster kennt kein `${…}` | [slice-167](../planning/done/wellenlos/slice-167-dependabot.md) |
| 2026-08-28 | **Die Reichweite war messbar, nicht offen — und sie trägt nicht, wie hier zugesagt.** Der zweite Re-Evaluierungs-Trigger und §Konsequenzen führen als *„nicht gemessen"*, ob Dependabot die **indirekten** Go-Abhängigkeiten anfasst. Gemessen: **dreizehn** der vierzehn Befunde lagen indirekt (`x/crypto`, `x/net`), eine direkt (`go-git`); Version-Updates betrachten ohne `allow: dependency-type: all` **nur** die zwei direkten Requires; und der zweite Weg ist im Repository abgeschaltet (`automated-security-fixes` `enabled:false`, `vulnerability-alerts` HTTP 404). **In der ausgelieferten Fassung erreichte der Kanal die Fundklasse also nicht, für die er gebaut ist.** Behoben in der Konfiguration (`allow: dependency-type: all`, für `gomod` dokumentiert unterstützt); die zweite Hälfte ist ein **Repo-Setting** und steht als Vorbedingung in [`releasing.md`](../../user/releasing.md) | [slice-167](../planning/done/wellenlos/slice-167-dependabot.md) |
| 2026-08-28 | **Drei Zahlen und eine Folgepflicht korrigiert.** *„sechs `uses:`-Einträge"* — der eigene Sensor zählt **acht** (sieben extern, drei **verschiedene** Actions). *„die vierzehn lagen in indirekten Abhängigkeiten"* — **dreizehn** von vierzehn. *„ein einziges direktes Dependency"* — es sind **zwei**. Und die Folgepflicht *„die `ignore`-Einträge tragen ihren Grund in der Datei"* zielte ins Leere: die Konfiguration enthält **keinen** `ignore`-Schlüssel; gemeint war jeder **Ausschluss**, und die tragen ihren Grund | [slice-167](../planning/done/wellenlos/slice-167-dependabot.md) |
