...
| 2026-06-29 | Das in der Zeile darüber genannte `adr-immutable-check.sh` ist mit [ADR-0025](0025-codepaths-ignore-refs.md) (slice-054) **entfernt** (in v0.33.0 nur noch pfad-stabil gehalten); seine immutablen Inline-Referenzen sind als `codepaths.ignore-refs`-Tombstones deklariert. Policy und Gate weiterhin unverändert (laufen seit [ADR-0024](0024-vcs-immutable-gate.md) über das Modul `vcs`). |
