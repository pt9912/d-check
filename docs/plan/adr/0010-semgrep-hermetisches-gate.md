...
| 2026-08-27 | Scanner-Pin auf `semgrep/semgrep:1.175.0` gehoben (Update-Politik [ADR-0011](0011-digest-pins-build-gate-images.md) §4: die Hebung ist ein bewusster Commit, keine ADR-Frage). §Entscheidung Punkt 3 nennt die damals gepinnte Fassung im Indikativ und ist insoweit **überholt**; die Entscheidung selbst — netzloser Scan, gepinntes Image **und** gepinntes Regelset — gilt unverändert. Vor der Hebung gemessen: beide Fassungen fahren denselben Umfang (55 Regeln, 50 Dateien, 0 Befunde), weil das Regelset über `RULES_COMMIT` eigen gepinnt ist |
| 2026-09-03 | Scanner-Pin auf `semgrep/semgrep:1.176.0` gehoben (Nachtlauf-Fund `make freshness-semgrep`, dieselbe Update-Politik). Gemessen nach der Hebung: `make semgrep` weiterhin 55 Regeln, 62 Dateien, 0 Befunde — unveränderter Umfang, weil das Regelset über `RULES_COMMIT` eigen gepinnt bleibt |
