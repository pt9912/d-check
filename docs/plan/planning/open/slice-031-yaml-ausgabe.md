...
- [ ] **CLI:** `--yaml` (boolesches Flag); Kombi-Check: `--json`+`--yaml`
  → Exit 2, `--repair`+`--yaml` → Exit 2; `--doctor`+`--yaml` erlaubt.
  render-Switch um `--doctor --yaml` und `--yaml` ergänzt.
- [ ] **Renderer:** YAML im report-Adapter über `gopkg.in/yaml.v3`
...
- [ ] **arch-check:** `tools/arch-check.sh` R3 erlaubt yaml.v3 zusätzlich
  in `internal/adapter/driven/report`; Kommentar/Regel-Text nachgezogen.
- [ ] **Spezifikation:** [§2 JSON-Ausgabe](../../../../spec/spezifikation.md#json-ausgabe---json)
  um den YAML-Hinweis (gleiche Struktur, nur Serialisierung) ergänzt.
- [ ] **Tests:** `--yaml` (parsbar, gleiche Struktur wie JSON, Exit 1);
...
- [ ] **Doku:** `docs/user/operations.md` (Options-Tabelle) + Handbuch
  (§4.11 Maschinenlesbare Ausgabe) um `--yaml` ergänzt.
- [ ] **[ADR-0009](../../adr/0009-yaml-im-report-adapter.md)** auf `Accepted` (mit diesem Slice); `make gates` grün;
