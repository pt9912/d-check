...
| [slice-115](in-progress/slice-115-arc-vergabe-architektur.md) | `ARC-*`-Vergabe in `spec/architecture.md` (§1 Kästen mit Komponenten-Tabelle, §2 Rollen nennen dieselben, §3 extern setzt fort; §5 kennungslos nach Vorlage) + `diagrams` opt-in auf die Architektur-Datei + `matrix`-Messung | Architektur §1–§5, Modul `diagrams` |
