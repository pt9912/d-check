...
| [`docs/plan/adr/0004-architektur-pattern-hexagonal.md`](../../adr/0004-architektur-pattern-hexagonal.md) | neu | sprachunabhängig (parallel zu `ADR-0001` entscheidbar); Zielbild für slice-002-Architektur und `arch-check`-Fitness-Function |
...
- **Was hat funktioniert:** Alle vier Entscheidungen ließen sich direkt
  gegen die `DC-QA`-NFAs führen; `ADR-0004` zuerst zu entscheiden
...
- **Anders als geplant:** Zusätzlich zum geplanten `AGENTS.md`-Update
  ...
  (folgt aus §3.1, stand nicht in der Plan-Tabelle). `ADR-0002` legt
...
- **Folge-Slices:** keine neuen; slice-002 ist durch `ADR-0001`…0004
