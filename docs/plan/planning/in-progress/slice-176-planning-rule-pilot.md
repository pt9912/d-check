...
3. **Der Preis der Symlink-Form ist benannt: die Dateien werden nicht
   ...
   Stelle:** beim Bump.
4. **Der Bump bekommt den neuen Träger.**
   [`MR-021`](../../../../harness/conventions.md#mr-021) zieht heute
   vendored-Pfad-**Links** auf den neuen Tag. Symlinks sind ein zweiter Träger
   derselben Bindung, und sie brechen **alle auf einmal** — gemeldet von
   niemandem: `links` sieht sie nicht (nicht gescannt), `make baseline-verify`
   auch nicht (es prüft die gepinnten Dateien, nicht Aliase darauf). Die
   Bump-Prozedur nennt sie ab jetzt ausdrücklich.
...
- [ ] **Der Bump-Träger ist geführt:** die Bump-Prozedur nennt die Symlinks
      unter `.claude/rules/` neben den vendored-Pfad-Links, mit dem Grund, dass
      sie kein Sensor meldet.
...
- **Die Symlinks sind nicht gate-geprüft** (gemessen, §2 Punkt 3). Ein Bump
  bricht sie stumm; die Bump-Prozedur ist die einzige Sicherung, und sie ist
  eine **Prozedur**, kein Sensor. — **Ausgang:** *(bei Closure)*
