...
ist über einen opt-in `trace`-Config-Block quell-/kennungs-konfigurierbar (die
vier Konventions-Achsen — Anforderungs-Quelle + Kennungs-Regex; je Referenzklasse
...
Accepted). Anlass: der Konsument grid-gym sah via `make doc-trace` nur 6 von 243
...
Modullisten-Restprüfung (Prüfung 3, Netzlos-Gate-Integrität) ist jetzt ein
getippter `configyaml.Decode`-Go-Test unter `make test`, `make gate-consistency`
fährt nur noch den `targets`-Modul-Dogfood — der `tools/*.sh`-Audit ist restlos
abgeschlossen. [ADR-0032](../../adr/0032-gate-consistency-tombstone.md) (Accepted;
revidiert die „Skript bleibt"-Konsequenz von
[ADR-0031](../../adr/0031-targets-deklarations-konsistenz-modul.md)). Impl-Review
R1 ACCEPT (MEDIUM Bindungsspalte eingearbeitet); kein Lastenheft-CR, kein Release
(Image byte-identisch), `make ci` grün
([Closure](../done/slice-064-gate-consistency-tombstone.md#7-closure-notiz-nach-done))).
Davor **welle-54-suggest-ai-harness-modulset**
...
Umgebungs-Drift wird am Entstehungsort gefangen). Dritte VCS-Port-Nutzung (`vcs`: Range-Diff,
`commits`: Messages, `tracked`: **Index**, ohne Range), Index-Wahrheit statt
`.gitignore`-Interpretation (gestagt = getrackt), kein Doppelbefund (`target-missing` bleibt
`links`; Verzeichnis-/Symlink-Ziele kein Kandidat), Ventil `tracked.exempt-targets`
(aufgelöster Zielpfad, segmentweise validiert), fail-closed ohne `.git`.
[ADR-0030](../../adr/0030-tracked-referenz-ziele.md) (Accepted),
[`DC-FA-TRK-001`](../../../../spec/lastenheft.md#dc-fa-trk-001--getrackt-status-auflösbarer-referenz-ziele-modul-tracked-opt-in)
(Lastenheft 0.37.1; zugleich
...
(Datei ohne Marker-Zeile + ohne `immutable.exclude-sections`) gehasst, Abweichung
→ `core-drift`; hermetisch (kein git, read-only-Arbeitsbaum), diagnose-only,
opt-in pro Datei; die git-historienbasierte Diff-Form bleibt einem späteren
VCS-Adapter vorbehalten
...
`matrix-downward`; additiv zu den Klassen-Paar-Regeln, fail-closed-Config,
...
Ziel-Span (Datei/Heading-Section, inkl. Fences) gehasst, Drift → `link-stale`;
nur auflösbare Ziele (struktureller Befund bleibt `links`/`anchors`),
diagnose-only
