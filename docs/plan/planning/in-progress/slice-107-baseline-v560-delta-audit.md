...
| Regel | Antwort | Befund |
|---|---|---|
| §Was ein Kommentar trägt (fünf Klassen, Adressaten-/Zeitform-Test, drei Verbots-Klassen) | konform gelebt, **mit gemessener Rest-Klasse** → C-3 | die Disziplin (Funktion statt Review-Historie) ist etabliert — aber die Stichprobe des Reviews fand nackte **Review-Finding-Tokens** als Herkunfts-Feld (u. a. `cli.go`, `planning.go`, dazu die in welle-77 geschriebenen in `structure_tableorder.go`/`markdown.go` aus welle-77): repo-weit mehrdeutig, keine der drei zugelassenen Herkunfts-Formen. C-3 räumt die Fundstellen (Review-Befund F-9) |
...
| **Widerspruchs-Ausgang** des Freshness-Audits (v5.4.0-Regel-Ergänzung: ein Widerspruchs-Fund wird **übernommen oder als deklarierte Abweichung behalten**, mit der Carveout-Abgrenzung dazwischen) | **konform als Verfahren — in diesem Audit angewandt** | genau dieser Ausgang entscheidet den Struktur-ID-Fall oben (MR-027 statt stiller MR-000-Lüge) <!-- d-check:ignore -->; Review-Befund F-1: die Zeile fehlte ausgerechnet für die Einordnungs-Regel der eigenen Abweichungs-Fälle |
...
| §Vergabe-Ergänzungen: die **Welle** zählt repo-weit dicht (kein Sub-Area-Segment); „lokal ableitbar" hat eine Grenze — die nächste Nummer liest Verzeichnis **und** offene Welle-Dateien (auch §4-Zeilen ohne Datei vergeben Nummern) | **konform** | Verfahren übernommen: die Nummern-Ableitung liest Verzeichnis **und** offene Welle-Dateien. Ein historischer Beleg-Fall liegt nicht vor — bei slice-108/109 entstanden §4-Zeile und Datei im selben Commit (Re-Review-Befund RF-3: der erste Beleg-Anspruch war nicht nachvollziehbar) |
| §Vergabe-Reste: die `MR`-Hybrid-Kollisionsklasse (Datei still, Index-Zeile laut) und die Abgrenzung **Ableiten ≠ Beanspruchen** | n. a. (operativ) / konform | Ein-Schreiber-Betrieb ohne Kollisions-Szenario; das Beanspruchen (Move auf dem Hauptzweig vor der Arbeit) wird ohnehin gelebt |
