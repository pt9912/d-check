...
## Messung (2026-09-08, slice-216) — korrigiert nach Review-Runde 1

**Die erste Fassung dieses Abschnitts behauptete Äquivalenz und lag falsch.**
Sie schrieb, `refs: ["**"]` erreiche die Wirkung von `exempt-paths` *„exakt"*
und *„und mehr"*. Der unabhängige Review hat die N+1-te Form gefunden und am
Code belegt. Was bleibt, ist schmaler und trägt trotzdem.

### Der beschriebene Fall: ausdrückbar, und schärfer

Drei Konfigurationen über einen Mini-Bestand — eine eingefrorene Datei mit
**zwei** toten Verweisen (einer auf einen entfernten Baseline-Baum, einer
**ohne** jeden Baseline-Bezug) und ein lebendes Dokument mit demselben
Baseline-Verweis:
...
**B ist das Rezept**, und es wirkt **auf beiden Achsen**: separat gemessen mit
`--enable anchors` — ein Anker in einen entfernten Baum ergibt ohne Ventil
einen Befund, mit Ventil **null**.

### Die N+1-te Form: `refs` ist **nicht** `exempt-paths`

**Die Symlink-Ablehnung überlebt jedes `refs`-Glob**, auch `refs: ["**"]`. Der
Grund steht im Produkt selbst
(`internal/hexagon/core/rules/links.go`, Kommentar über dem Ventil):

> wird nicht escape-/existenz-geprüft; die Symlink-Prüfung (oben, **Vorrang**)
> bleibt unberührt.

**Die beiden Ventile stehen damit quer zueinander, nicht in Dominanz.** Ein
`exempt-paths` nähme die Datei aus dem **ganzen Modul**; `ignore-refs` nimmt
**Auflösungs-Klassen eines Ziels** heraus. Für den Fall des CR fallen sie
zusammen — für den allgemeinen Fall nicht, und die erste Fassung hat das
Gegenteil behauptet.

**Und eine Divergenz zwischen Spec und Produkt fällt dabei ab, älter als
dieser CR:**
[`DC-FA-REF-001`](../../../spec/lastenheft.md#dc-fa-ref-001--geteiltes-referenz-ventil-ignore-refs-mit-quell-skopus)
sagt, hinter dem Ventil blieben *„Symlink-Ablehnung, Repo-Escape u. Ä."*
stehen. Gemessen bleibt die **Symlink**-Ablehnung; der **Repo-Escape** nicht —
das Ventil greift davor. **Das ist kein Befund dieses CR und wird hier nur
benannt**; es gehört als eigener Vorgang aufgenommen.

---

## Entscheid (2026-09-08, Begründung nach Review-Runde 1 überarbeitet)

**Der CR wird nicht umgesetzt.** `links` und `anchors` bekommen kein
`exempt-paths`. **Zwei der drei Argumente treffen zu**, und ein Grund der
ersten Fassung ist gestrichen, weil er am eigenen Bestand widerlegt ist.

### Antwort auf (a) — *„kein Knopf"*

**Die Prämisse trifft nicht zu, für den Fall des CR.**
[`DC-FA-REF-001`](../../../spec/lastenheft.md#dc-fa-ref-001--geteiltes-referenz-ventil-ignore-refs-mit-quell-skopus)
nennt `links` und `anchors` ausdrücklich, und Form **B** löst den beschriebenen
Fall — auf beiden Achsen gemessen. **Nicht zutreffend wäre die Umkehrung:**
Der Knopf ist **kein** Ersatz für `exempt-paths` im Allgemeinen (Symlink).
Richtig ist zusätzlich, dass die beiden Module keine **modul-lokale**
Options-Sektion führen.

### Antwort auf (b) — das Konsistenz-Argument

**Es trifft zu, und die erste Fassung hat es doppelt falsch behandelt.**

**Erstens die Zahl:** Es sind **acht** Module mit `exempt-paths`, nicht sechs —
`ids.patterns[]` und die `structure`-Regeln fallen aus dem Grep-Muster des CR
heraus, und der eigene Entscheids-Text nannte `ids` im selben Absatz als
Beispiel für eine zweite Achse, ohne zu bemerken, dass diese Achse genau
`exempt-paths` **ist**.

**Zweitens der gestrichene Grund.** Die erste Fassung argumentierte, die
Datei-Achse sei *„die bequemere"* und würde die schärfere verdrängen. **Das ist
am eigenen Bestand widerlegt:** Dieses Repo nimmt `docs/reviews/**` für
`codepaths` über die **Datei**-Achse aus — obwohl dort alle drei Achsen zur
Verfügung stehen, und für **dieselbe** Artefakt-Klasse, um die es im CR geht.
Wer das tut und dem Adopter die Datei-Achse mit dem Bequemlichkeits-Argument
verweigert, misst mit zwei Maßen. **Der Grund ist gestrichen.**

**Was die Ablehnung dann noch trägt** — schmaler, aber gemessen: Für den
beschriebenen Fall ist die schärfere Form **verfügbar und eine Zeile lang**,
und `links` ist das Kern-Modul, in dem ein datei-weiter blinder Fleck am
meisten kostet. Ein zweiter Knopf wäre gerechtfertigt durch einen Fall, den
die vorhandene Achse **nicht** trägt — und der ist bisher nicht gezeigt.

### Antwort auf (c) — die drei verworfenen Wege

- **`scan.ignore`: zugestimmt.** Falsches Instrument, Begründung des CR richtig.
- **Koexistenz des alten Baums: zugestimmt.**
- **`ignore-refs`-Breite: nicht beurteilbar von hier.** Der Grund war ein
  Breiten-Wächter im Repo des Absenders. **Die erste Fassung schrieb dazu
  *„dieses Repo fährt 25 Einträge ohne Konflikt"* — das misst die Abwesenheit
  einer Regel, nicht ihre Erfüllung:** Dieses Repo hat gar keinen
  Breiten-Wächter; ein CR dazu wurde einen Tag zuvor abgelehnt. Der Satz ist
  gestrichen. **Was beiträgt, ist die Form:** **B** ist *eine* `refs:`-Zeile
  mit *einem* Glob, kein Paar je Ziel — ob die Kappung sie trifft, wäre neu zu
  messen, und das kann nur der Absender.
...
Deckt `links` **und** `anchors` (beide gemessen), gilt nur in den genannten
Dateien, lässt jeden anderen **Auflösungs**-Defekt darin sichtbar — und die
Symlink-Ablehnung ohnehin.

### Was den Entscheid umkehren würde

1. **Ein Fall, den `ignore-refs` nicht trägt und der im Bestand des Absenders
   vorkommt.** Die Symlink-Klasse ist ein solcher Fall **im Allgemeinen** — sie
   ist damit **kein** hypothetischer Trigger mehr, sondern einer, dem eine
   Population fehlt. Nachmessbar: `d-check --enable links` über
   `docs/reviews/**` mit dem Grund-Code `symlink`.
2. **Der Breiten-Wächter erweist sich als Eigenschaft des Werkzeugs**, nicht
   des Absender-Repos.
3. **Die Ziel-Achse bekommt eine Einschränkung, die die Datei-Achse nicht
   hätte** — dann fiele die Rechnung anders aus, ohne dass jemand den CR
   erneut stellen müsste.

**Kein Sensor wacht über die drei.** Sie sind Wiedervorlage-Bedingungen, kein
Trigger mit Wächter. **Und die Rückfrage bleibt offen**, die dieser Entscheid
nicht ersetzt: ob nach Form **B** ein Fall übrig bleibt. Der Entscheid ist ohne
sie gefallen — auf Auftraggeber-Entscheid, und das steht hier statt es zu
verschweigen.
