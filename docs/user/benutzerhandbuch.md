...
**Handbuch-Version:** 1.35 · **Software-Version:** [v0.47.0](../../version.md#v0.47.0) ·
...
```bash
docker pull ghcr.io/pt9912/d-check:v0.47.0
```
...
- `:v0.47.0` — eine feste Version (empfohlen für reproduzierbare Läufe; die jeweils
...
docker run --rm -v "$PWD:/repo:ro" \
  ghcr.io/pt9912/d-check@sha256:ad42432df71ef747cc8cefc92591339afe78fe707149b674cf086be3ff423eed
```
...
```bash
docker run --rm -v "$PWD:/repo:ro" ghcr.io/pt9912/d-check:v0.47.0
```
...
docker run --rm --network none -v "$PWD:/repo:ro" \
  ghcr.io/pt9912/d-check:v0.47.0
```
...
```bash
docker run --rm ghcr.io/pt9912/d-check:v0.47.0 --print-config > .d-check.yml
```
...
```bash
docker run --rm -v "$PWD:/repo:ro" ghcr.io/pt9912/d-check:v0.47.0 --json
```
...
```bash
docker run --rm -v "$PWD:/repo:ro" ghcr.io/pt9912/d-check:v0.47.0 --yaml
```
...
```bash
docker run --rm -v "$PWD:/repo:ro" ghcr.io/pt9912/d-check:v0.47.0 --trace
```
...
```bash
docker run --rm ghcr.io/pt9912/d-check:v0.47.0 --print-mk > d-check.mk
...
```text
DCHECK_IMAGE ?= ghcr.io/pt9912/d-check:v0.47.0
DCHECK_DIGEST ?=
TRACE_FLAGS ?=
