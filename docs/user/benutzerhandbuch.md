...
**Handbuch-Version:** 1.64 · **Software-Version:** [v0.70.0](../../version.md#v0.70.0) ·
**Stand:** 2026-08-30 · **Autor:** pt9912
...
```bash
docker pull ghcr.io/pt9912/d-check:v0.70.0
```
...
```bash
docker pull pt9912/d-check:v0.70.0
```

### Versionen und Tags

- `:v0.70.0` — eine feste Version (empfohlen für reproduzierbare Läufe; die jeweils
...
docker run --rm -v "$PWD:/repo:ro" \
  ghcr.io/pt9912/d-check@sha256:dca9b3504e8d9dabfbe1a2c7ad975f5be10613328c0026981f5fcfd792f42f78
```
...
docker run --rm --network none -v "$PWD:/repo:ro" \
  ghcr.io/pt9912/d-check:v0.70.0
```
...
```bash
docker run --rm ghcr.io/pt9912/d-check:v0.70.0 --print-config > .d-check.yml
```
...
```bash
docker run --rm -v "$PWD:/repo:ro" ghcr.io/pt9912/d-check:v0.70.0 --json
```
...
```bash
docker run --rm -v "$PWD:/repo:ro" ghcr.io/pt9912/d-check:v0.70.0 --yaml
```
...
```bash
docker run --rm -v "$PWD:/repo:ro" ghcr.io/pt9912/d-check:v0.70.0 --trace
```
...
```bash
docker run --rm ghcr.io/pt9912/d-check:v0.70.0 --print-mk > d-check.mk
...
DCHECK_IMAGE ?= ghcr.io/pt9912/d-check:v0.70.0
DCHECK_DIGEST ?=
TRACE_FLAGS ?=
...
```bash
docker run --rm --network none -v "$PWD:/repo:ro" ghcr.io/pt9912/d-check:v0.70.0 \
