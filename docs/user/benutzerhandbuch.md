...
**Handbuch-Version:** 1.73 · **Software-Version:** [v0.76.1](../../version.md#v0.76.1) ·
...
```bash
docker pull ghcr.io/pt9912/d-check:v0.76.1
```
...
```bash
docker pull pt9912/d-check:v0.76.1
```

### Versionen und Tags

- `:v0.76.1` — eine feste Version (empfohlen für reproduzierbare Läufe; die jeweils
...
```bash
docker run --rm -v "$PWD:/repo:ro" ghcr.io/pt9912/d-check:v0.76.1
```
...
docker run --rm --network none -v "$PWD:/repo:ro" \
  ghcr.io/pt9912/d-check:v0.76.1
```
...
```bash
docker run --rm ghcr.io/pt9912/d-check:v0.76.1 --print-config > .d-check.yml
```
...
```bash
docker run --rm -v "$PWD:/repo:ro" ghcr.io/pt9912/d-check:v0.76.1 --json
```
...
```bash
docker run --rm -v "$PWD:/repo:ro" ghcr.io/pt9912/d-check:v0.76.1 --yaml
```
...
```bash
docker run --rm -v "$PWD:/repo:ro" ghcr.io/pt9912/d-check:v0.76.1 --trace
```
...
```bash
docker run --rm ghcr.io/pt9912/d-check:v0.76.1 --print-mk > d-check.mk
...
```bash
docker run --rm --network none -v "$PWD:/repo:ro" ghcr.io/pt9912/d-check:v0.76.1 \
...
- **Der Ort ist konfigurierbar, nicht verdrahtet.** `.github/workflows` ist
  ...
  wertlos.
- **Derselbe SHA trägt überall denselben Tag-Kommentar — dateiübergreifend.**
  Steht ein SHA an zwei Stellen mit unterschiedlichem Kommentar (`# v4.2.0` an
  der einen, `# v3.6.0` an der anderen), meldet `uses-pin-tag-conflict` **jede**
  beteiligte Zeile, mit beiden Werten in der Meldung. Welcher stimmt, sagt die
  Prüfung nicht — das wäre die Gültigkeitsfrage von oben. **Benannte Grenze:**
  zwei *legitime* Tags auf demselben Commit (ein Major-Tag `v4` neben seinem
  Patch-Release `v4.2.0`) sehen textuell verschieden aus und lösen denselben
  Befund aus, obwohl beide wahr sind — diese Unterscheidung trifft die Prüfung
  nicht.
...
**Verzeichnis-Register statt Tabellendatei (`observations.dir`).** Statt
einer Tabelle kann Ihr Register eine Ablage aus Verzeichnissen sein — je
Beobachtung ein Ordner `<KUERZEL>/<slug>/`, dessen `observation.md` die
Kennung trägt. „Deklariert" heißt hier: das Verzeichnis
`<observations.dir>/<zitierter-pfad>/observation.md` existiert.
...
planning:
  observations:
    ...
    dirs: [docs/plan/planning]
    # pattern: 'BEO-[A-Z][A-Z0-9]*/[a-z][a-z0-9-]*'   # Default für diesen Modus
```

`register` und `dir` sind **mutually exclusive** — sind beide gesetzt, bricht
der Lauf mit Exit 2. Ohne beide bleibt die Fähigkeit inert. Die übrige
Prüf-Logik (Fließtext/Linktext zählen, Inline-Code-Beispiele nicht, nur die
...
`matrix.classes[].paths`/`.order`, `mentions.artifacts`/`.documents` und die
`exempt-paths` der Module.

**Eine Ausnahme, gemessen und benannt:** Die `exempt-paths` der Module
`workflows` und `reviews` werden mit **blankem** `path.Match` geprüft — dort
zählt nur die **Segmentzahl**, und `**` verhält sich wie `*`. Das fällt in der
Praxis nicht auf: Beide Module listen ihr Verzeichnis **nicht rekursiv**, ihre
Kandidaten liegen also immer genau eine Ebene tief, und ein Muster der Form
`<dir>/<datei>` trifft sie. Wer dort ein Muster über mehrere Ebenen schreibt,
bekommt kein Ergebnis — nicht, weil das Ventil versagt, sondern weil es dort
nichts Tieferes gibt. `scan.ignore`, `matrix` und `mentions` verwenden die
volle Semantik oben.
...
`Range-Spitze "<ref>" nicht auflösbar: nicht vollständig lesbarer Tree zu
"<ref>": nicht lesbarer Unterbaum "<name>": object not found` oder
...
> **Wichtig, wenn Sie ein Image bis `v0.76.0` pinnen.** Bis einschließlich
> `v0.75.0` **schwieg** dieser Fall vollständig, sobald der unsichtbare Pack
> nur *einzelne* Objekte verschluckte — und genau solche partiellen Packs
> hinterlässt `git maintenance`, weil es in Stapeln arbeitet. Eine echte
> Core-Änderung wurde dann **nicht gemeldet**: `0 Befunde`, Exit 0. **`v0.76.0`
> behob zwei von vier Ausprägungen** — das unsichtbare **Blob** der Datei
> selbst und ihr unsichtbares **Verzeichnis-Objekt, sofern die Gegenseite
> noch einen Vergleichspunkt trug**. **Zwei blieben bis einschließlich
> `v0.76.0` offen:** ein unsichtbares Verzeichnis-Objekt **ohne** Pendant auf
> der Gegenseite (das Verzeichnis wurde komplett gelöscht) meldete weiterhin
> `0 Befunde`/Exit 0, und ein unsichtbarer **HEAD**-Tree meldete
> **fälschlich** `core-drift-vcs` „gelöscht oder umbenannt" (Exit 1) statt
> des eigentlichen Umgebungsfehlers. Ursache war durchgehend dieselbe
> Bibliotheks-Eigenschaft: ein *unlesbares* Objekt wird mit demselben Fehler
> gemeldet wie *„diese Datei existiert in diesem Stand nicht“* — ein
> legitimer Zustand, den die Prüfung überspringen muss —, und ein Tree-Diff
> verwandelt einen nicht ladbaren Unterbaum zusätzlich in ein stilles Ende
> der Aufzählung, statt einen Fehler zu melden. **Ab `v0.76.1` sind alle
> vier Ausprägungen behoben:** die geschützte Pfad-Menge wird jetzt direkt
> gegen beide vollständigen Tree-Stände aufgelöst statt gegen einen Diff, und
> ein nicht ladbarer Unterbaum bricht die Auflösung selbst ab, bevor
> irgendein Pfad klassifiziert wird. Solange Sie `v0.76.0` oder älter
> pinnen, prüfen Sie einmalig mit `ls .git/objects/pack/`, dass jede
> `.pack`-Datei mit `pack-` beginnt.
...
**Grenze — dieselbe Pack-Namensform wie bei `vcs`, aber mit besserem
Ausgang.** Weil `commits` denselben git-Port benutzt, ist auch hier ein Pack
unter einem anderen Namen als `pack-<Hash-des-Packs>.{idx,pack}` unsichtbar
(Einzelheiten oben bei
[`vcs`](#immutabilität-über-eine-commit-range-prüfen-modul-vcs)). Sie sehen
dann `Range-Basis "<base>" nicht auflösbar: reference not found` oder
...
**Vier Grenzen, damit Sie nicht überrascht werden.** Der **Fence** bleibt außen
...
**Die vierte Grenze ist still, und deshalb steht sie hier und nicht im
Kleingedruckten:** ein **vergessener Schluss-Fence** blendet alles Folgende aus.
Die Bedingung sieht dann nichts mehr und meldet grün — ohne geprüft zu haben.
`max-open-tasks` behebt das **nicht** und soll es nicht; es löst den
Backtick-Ausfall, nicht den Fence-Ausfall. Wenn diese Regel bei Ihnen über einen
Übergang entscheidet, schalten Sie im **selben Profil** das Modul `spans` dazu:
`fence-unclosed` ist der Sensor, der die offene Fence meldet. Ohne ihn haben Sie
einen Wächter, den ein einzelnes Zeichen abschaltet.
...
| 1.62             | v0.68.0          | 2026-08-30 | **Ein Befund sagt jetzt auch, was zu tun ist — und eine `structure`-Regel darf ihre Grundmenge erklären** (Lastenheft 0.75.0 und 0.76.0). **Erstens erreicht die Erläuterung eines Befunds den Menschen** (§4.20, §5): das Feld `message` gab es seit jeher, gerendert wurde es aber nur in `--json`/`--yaml`. Es steht jetzt als **vierte** tab-getrennte Spalte der Befund-Zeile — **nur wenn gefüllt**, ein Befund ohne Erläuterung bleibt byte-identisch dreispaltig — und als eigene `Hinweis:`-Zeile in `--doctor`. Neu dazu `structure[].hint`: eine Regel **verfasst** ihre Erläuterung selbst und sagt damit, welche **Zusage** sie hütet, während der Grund-Code die **Art** des Defekts sagt. Ein `hint` mit Tab oder Zeilenumbruch wird **abgewiesen** (Exit 2), sonst wäre die Zusage „ein Befund pro Zeile" über fremden Text brechbar. **Abgegrenzt gegen den Fix-Kandidaten** (`--repair`): der ist abgeleitet, der Hinweis ist verfasst und wird nie angewendet. **Zweitens darf eine `structure`-Regel ihre geprüfte Menge erklären** (§5, zwei neue Schlüssel, beide opt-in, beide **verkleinern** nur, kein neuer Grund-Code): `tasks-ignore-pattern` nimmt Task-Items aus der `max-tasks`-Zählung, `exempt-section-pattern` nimmt **Abschnitte** aus der Regel — das Geschwister von `exempt-paths` eine Stufe tiefer, für Bestände **innerhalb einer Datei**. **Die beiden Muster sehen verschiedene Zeichenketten, und das ist Absicht:** das Abschnitts-Muster dieselbe rohe Überschriften-Zeile wie `section-pattern` (samt `#`-Folge — ein analog geschriebenes Muster soll nicht still danebengreifen), das Item-Muster den **Item-Text hinter der Checkbox** auf dem **bereinigten** Text, damit `^` „das Item beginnt so" bedeuten kann. **Zwei gemessene Fallen stehen im Handbuch, weil beide beim ersten Versuch zuschlagen:** ein freies Substring-Muster nimmt Punkte mit, die die Wendung nur erwähnen, und ein Muster auf einen Ausdruck in **Backticks** trifft Leerzeichen — die Zählung liest den bereinigten Text. Die Diagnose steht in der Meldung selbst: sie nennt die Zahl der ignorierten Items, **auch bei null**. Ohne beide Schlüssel ist der Befundsatz byte-identisch |
| 1.63             | v0.69.0          | 2026-08-30 | **Eine erklärte Teilmenge darf die geprüfte Menge leeren — wenn sie sagt, wie viele** (Lastenheft 0.77.0 und 0.78.0). Seit `v0.68.0` meldet eine `structure`-Regel `section-missing`, wenn `exempt-section-pattern` **alle** Treffer abzieht — und das warf **zwei** Zustände in einen Grund-Code: den Konfigurationsdefekt (der Selektor trifft nichts) und den Bestandszustand (die Ausnahme nimmt alle, *„noch nichts Neues zu prüfen"*). Der neue Schlüssel **`exempt-expect-count`** (§5, int ≥ 0, opt-in, nur mit dem Muster) **deklariert**, wie viele Abschnitte die Ausnahme nehmen soll: stimmt die Zahl, ist die geleerte Menge **kein** Befund; weicht sie ab, meldet der neue Grund-Code **`section-exempt-mismatch`** mit beiden Zahlen. **Der Konfigurationsdefekt bleibt `section-missing`**, auch mit gesetzter Zahl. Geprüft wird **in beide Richtungen und immer**, nicht nur bei geleerter Restmenge — eine erweiterte Aufzählung ohne nachgezogene Zahl ist dieselbe Lücke wie eine veraltete. **`0` ist erlaubt und bedeutet etwas** und bleibt von einem abwesenden Schlüssel unterscheidbar. **Drei Dinge überraschen und stehen deshalb beim Schlüssel:** er gilt **je Datei** (ein Glob über zehn Dateien hält die Zahl gegen jede einzeln), sein Befund **bricht die Datei ab**, und er ist **fail-closed** — ohne das Muster oder mit negativem Wert endet der Lauf mit Exit 2. **Der Preis steht dabei:** die Zahl altert wie jeder Autoren-Text, und wer sie blind mitzieht, hat einen Wächter, der nur noch sich selbst bestätigt. **Zweitens ist das Handbuch aus dem Werkzeug heraus auffindbar:** `--help` nennt seine URL, und das per `--print-mk` erzeugte Fragment trägt sie im **Kopfkommentar** — es reist in ein fremdes Repo, und sein Kopf ist der einzige Ort, an dem ein Zeiger dorthin dauerhaft mitfährt. Die URL zeigt auf den **Hauptzweig** ohne Versionsangabe: sie kann nicht veralten, zeigt aber auch nie auf den Stand eines gepinnten Images — wer die passende Fassung braucht, liest den Software-Versions-Stempel oben. Ohne den neuen Schlüssel ist der Befundsatz **byte-identisch** |
| 1.64             | v0.70.0          | 2026-08-30 | **Eine Bedingung zählt offene Task-Items auf dem ROHEN Abschnitt** (Lastenheft 0.79.0). Neu ist **`max-open-tasks`** (§5, int ≥ 0, opt-in): die Obergrenze der **offenen** Task-Items eines Abschnitts, gezählt auf den **rohen** Zeilen, mit dem eigenen Grund-Code **`section-tasks-open`** — ein Befund **je Haken auf seiner Zeile**, nicht einer je Abschnitt. **Der Unterschied zu `max-tasks` klingt klein und ist es nicht:** jenes liest den bereinigten Text, und weil die Inline-Code-Paarung **absatzweise** ist, macht ein einzelner überzähliger Backtick den Rest des Absatzes unsichtbar — gemessen fällt dieselbe Eingabe von einem Befund auf **null**. Für Prosa ist dieser Preis richtig; für einen Wächter, der über einen Übergang entscheidet, nicht. **Gezählt wird über die Lexik des Moduls:** alle vier Listen-Marker (`-`, `*`, `+`, geordnete Liste), eingerückt und mit Tabulator, verengt auf die leere Box — ein selbst geschriebenes `forbid-pattern: '- \[ \]'` deckt nur die Form, an die sein Autor gedacht hat. Die ersten `max-open-tasks` Items in Dokument-Reihenfolge sind erlaubt. **Vier Grenzen stehen im Handbuch, weil sie überraschen:** der Fence bleibt außen vor (ein Haken **im** wohlgeformten Fenced-Block ist damit unsichtbar); eine **einzeilige** Inline-Spanne meldet **nicht**, eine **mehrzeilige** zählt mit; Blockquote und Tabulator in der Box zählen für **keine** der beiden Bedingungen; und ein **vergessener** Schluss-Fence macht die Bedingung blind — wer sie als Vorbedingung fährt, schaltet `spans` im selben Profil dazu, dort meldet `fence-unclosed`. Ein negativer Wert ⇒ Exit 2. Ohne den Schlüssel ist der Befundsatz **byte-identisch** |
