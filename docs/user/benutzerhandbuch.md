...
**Handbuch-Version:** 1.60 · **Software-Version:** [v0.66.0](../../version.md#v0.66.0) ·
...
```bash
docker pull pt9912/d-check:v0.66.0
```

### Versionen und Tags

- `:v0.66.0` — eine feste Version (empfohlen für reproduzierbare Läufe; die jeweils
...
docker run --rm -v "$PWD:/repo:ro" \
  ghcr.io/pt9912/d-check@sha256:fde2b8098d021c225ba3e7bc1d49f3ead2f74cac4da7e3bb8095f448d6af9068
```
...
docker run --rm --network none -v "$PWD:/repo:ro" \
  ghcr.io/pt9912/d-check:v0.66.0
```
...
```bash
docker run --rm ghcr.io/pt9912/d-check:v0.66.0 --print-config > .d-check.yml
```
...
```bash
docker run --rm -v "$PWD:/repo:ro" ghcr.io/pt9912/d-check:v0.66.0 --json
```
...
```bash
docker run --rm -v "$PWD:/repo:ro" ghcr.io/pt9912/d-check:v0.66.0 --yaml
```
...
```bash
docker run --rm -v "$PWD:/repo:ro" ghcr.io/pt9912/d-check:v0.66.0 --trace
```
...
```bash
docker run --rm ghcr.io/pt9912/d-check:v0.66.0 --print-mk > d-check.mk
...
```text
DCHECK_IMAGE ?= ghcr.io/pt9912/d-check:v0.66.0
DCHECK_DIGEST ?=
TRACE_FLAGS ?=
...
```bash
docker run --rm --network none -v "$PWD:/repo:ro" ghcr.io/pt9912/d-check:v0.66.0 \
