...
```bash
docker pull ghcr.io/pt9912/d-check:v0.37.1
```
...
- `:v0.37.1` — eine feste Version (empfohlen für reproduzierbare Läufe; die jeweils
...
docker run --rm --network none -v "$PWD:/repo:ro" \
  ghcr.io/pt9912/d-check:v0.37.1
```
...
```bash
docker run --rm ghcr.io/pt9912/d-check:v0.37.1 --print-config > .d-check.yml
```
...
```bash
docker run --rm -v "$PWD:/repo:ro" ghcr.io/pt9912/d-check:v0.37.1 --json
```
...
```bash
docker run --rm -v "$PWD:/repo:ro" ghcr.io/pt9912/d-check:v0.37.1 --trace
```
...
```bash
docker run --rm ghcr.io/pt9912/d-check:v0.37.1 --print-mk > d-check.mk
