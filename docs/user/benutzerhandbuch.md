...
**Handbuch-Version:** 1.19 · **Software-Version:** [v0.37.0](../../version.md#v0.37.0) ·
**Stand:** 2026-07-03 · **Autor:** pt9912
...
```bash
docker pull ghcr.io/pt9912/d-check:v0.37.0
```
...
- `:v0.37.0` — eine feste Version (empfohlen für reproduzierbare Läufe; die jeweils
...
docker run --rm --network none -v "$PWD:/repo:ro" \
  ghcr.io/pt9912/d-check:v0.37.0
```
...
```bash
docker run --rm ghcr.io/pt9912/d-check:v0.37.0 --print-config > .d-check.yml
```
...
```bash
docker run --rm -v "$PWD:/repo:ro" ghcr.io/pt9912/d-check:v0.37.0 --json
```
...
```bash
docker run --rm -v "$PWD:/repo:ro" ghcr.io/pt9912/d-check:v0.37.0 --yaml
```
...
```bash
docker run --rm -v "$PWD:/repo:ro" ghcr.io/pt9912/d-check:v0.37.0 --trace
```
...
```bash
docker run --rm ghcr.io/pt9912/d-check:v0.37.0 --print-mk > d-check.mk
...
```text
DCHECK_IMAGE ?= ghcr.io/pt9912/d-check:v0.37.0
DCHECK_DIGEST ?=
TRACE_FLAGS ?=
