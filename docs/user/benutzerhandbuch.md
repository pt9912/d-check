...
**Handbuch-Version:** 1.8 · **Software-Version:** v0.26.0 ·
...
```bash
docker pull ghcr.io/pt9912/d-check:v0.26.0
```
...
docker run --rm -v "$PWD:/repo:ro" \
  ghcr.io/pt9912/d-check@sha256:68951f5a3dd7ad3404e1996d45327f3df2585c0ef2b0b6bde7ccf790da4ddf6a
```
...
```bash
docker run --rm -v "$PWD:/repo:ro" ghcr.io/pt9912/d-check:v0.26.0
```
...
docker run --rm --network none -v "$PWD:/repo:ro" \
  ghcr.io/pt9912/d-check:v0.26.0
```
...
```bash
docker run --rm ghcr.io/pt9912/d-check:v0.26.0 --print-config > .d-check.yml
```
...
```bash
docker run --rm -v "$PWD:/repo:ro" ghcr.io/pt9912/d-check:v0.26.0 --json
```
...
```bash
docker run --rm -v "$PWD:/repo:ro" ghcr.io/pt9912/d-check:v0.26.0 --yaml
```
...
```bash
docker run --rm -v "$PWD:/repo:ro" ghcr.io/pt9912/d-check:v0.26.0 --trace
```
...
```bash
docker run --rm ghcr.io/pt9912/d-check:v0.26.0 --print-mk > d-check.mk
...
```text
DCHECK_IMAGE ?= ghcr.io/pt9912/d-check:v0.26.0
TRACE_FLAGS ?=
