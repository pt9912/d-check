...
| `--config <datei>` | **ersetzt** für diesen Lauf die konventionelle `.d-check.yml` der Scan-Wurzel durch die genannte Datei (Pfad relativ zur Wurzel, muss **innerhalb** liegen; gleiche strikte Validierung). Ersetzt statt zu ergänzen — eine vorhandene `.d-check.yml` wird nicht gelesen und nicht zusammengeführt; damit fährt ein Repo mehrere Prüf-Profile (Inner-Loop vs. Closure). **Kein stiller Rückfall:** fehlende Datei, Verzeichnis-Ziel, Symlink-Ausbruch aus der Wurzel oder ein **leerer** Wert ⇒ Exit 2 ([`DC-FA-CLI-012`](../../spec/lastenheft.md#dc-fa-cli-012--konfigurations-pfad-überschreiben)) |
...
| `--print-mk` | include-bares `d-check.mk` auf stdout, dann Exit 0 — **kein Scan, schreibt nichts**; `DCHECK_IMAGE ?= ghcr.io/pt9912/d-check:v<version>` (eingebettete Release-Version; Digest via `DCHECK_IMAGE`- oder `DCHECK_DIGEST`-Override), die Variable `TRACE_FLAGS` und dreizehn `##`-annotierte `doc-*`-Targets — vom Befund-Gate `doc-check` über die Fokus-Targets der opt-in-Module bis zu `doc-usage` (Hilfe des Werkzeugs) und `doc-help` (Liste der Targets); die vollständige Aufzählung nennt das erzeugte Fragment selbst. Einbinden: `d-check --print-mk > d-check.mk`, dann `include d-check.mk` ([`DC-FA-CLI-010`](../../spec/lastenheft.md#dc-fa-cli-010--makefile-fragment-ausgeben)) |
...
| `--trace` | **Requirements Traceability Matrix** auf stdout: je Anforderung die referenzierenden ADRs/Slices + Waisen-Markierung — **liest, schreibt nichts**, kein Dokument erzeugt; Default Markdown-Tabelle, mit `--json`/`--yaml` maschinenlesbar; nicht mit `--doctor`/`--repair`. Anforderungsquelle als Heading (Default) oder konfigurierte Markdown-Pipe-Tabelle (`trace.requirements.format`/`table.*`); nichtleer explizite Quelle oder Tabellenmodus ohne erkannte Anforderung ⇒ Exit 2. Weitere Quellen per `trace`-Block konfigurierbar — ohne Block byte-identisch. Der opt-in `trace.cross-consistency`-Block vergleicht zusätzlich zwei Traceability-Sichten (Vorwärts-RTM-Tabelle gegen Rückwärts-`Bezug`-Kanten) und meldet je Anforderung beide Mengendifferenzen als **eigenen** Abschnitt unter der RTM (keine RTM-Spalte); advisory (Exit 0), fail-closed inkl. vakuumem Abgleich ([`DC-FA-CLI-009`](../../spec/lastenheft.md#dc-fa-cli-009--requirements-traceability-matrix), [`DC-FA-REQ-001`](../../spec/lastenheft.md#dc-fa-req-001--anforderungsquellen-als-headings-oder-tabellen), [`DC-FA-XREF-001`](../../spec/lastenheft.md#dc-fa-xref-001--kreuzverweis-konsistenz-zweier-traceability-sichten-tracecross-consistency-opt-in)) |
| `--require-complete` | nur mit `--trace`: bei ≥1 Requirements-Waise **Exit 1** statt 0 (opt-in Vollständigkeits-Gate; die RTM bleibt unverändert auf stdout, der Default-`--trace` bleibt advisory Exit 0) — bei aktivem `trace.cross-consistency` **ebenso bei ≥1 Kreuzverweis-Differenz** (beide Ursachen melden getrennt auf stderr). Ohne `--trace` Nutzungsfehler (Exit 2) ([`DC-FA-CLI-011`](../../spec/lastenheft.md#dc-fa-cli-011--vollständigkeits-prüfung-als-opt-in-exit-code), [`DC-FA-XREF-001`](../../spec/lastenheft.md#dc-fa-xref-001--kreuzverweis-konsistenz-zweier-traceability-sichten-tracecross-consistency-opt-in)) |
| `--range <base>..<head>` | Module `vcs`/`commits`: prüft die git-Commit-Range statt des Arbeitsbaums (`vcs`: Core-Immutabilität über die Range, `commits`: Traceability-Kennung jeder Commit-Message); fail-closed ohne `.git` ([`DC-FA-VCS-001`](../../spec/lastenheft.md#dc-fa-vcs-001--git-diff-immutabilität-des-core-über-eine-commit-range-modul-vcs-opt-in), [`DC-FA-COMMITS-001`](../../spec/lastenheft.md#dc-fa-commits-001--traceability-kennung-in-commit-messages-über-eine-commit-range-modul-commits-opt-in)) |
...
der [Spezifikation §`.d-check.yml`](../../spec/spezifikation.md#spec-005--d-checkyml),
