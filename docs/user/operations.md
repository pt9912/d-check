...
| `--print-mk` | include-bares `d-check.mk` auf stdout, dann Exit 0 — **kein Scan, schreibt nichts**; `DCHECK_IMAGE ?= ghcr.io/pt9912/d-check:v<version>` (eingebettete Release-Version; Digest via `DCHECK_IMAGE`-Override), eine `TRACE_FLAGS`-Variable und die Targets `doc-check`, `doc-trace` (advisory RTM) und `doc-complete` (Vollständigkeits-Gate). Einbinden: `d-check --print-mk > d-check.mk`, dann `include d-check.mk` ([`DC-FA-CLI-010`](../../spec/lastenheft.md#dc-fa-cli-010--makefile-fragment-ausgeben)) |
