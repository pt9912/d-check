...
| `--doctor` | erklärende, nach Datei gruppierte Diagnose auf stdout statt der Befund-Zeilen, mit Fix-Kandidaten wo eindeutig (v1: `id-unlinked` → Definitions-Link) — **liest, schreibt nichts**; mit `--json` kombinierbar (maschinenlesbare Diagnose: `findings` zusätzlich mit `reasonText`/`fixCandidate`), nicht mit `--repair` ([`DC-FA-CLI-007`](../../spec/lastenheft.md#dc-fa-cli-007--diagnose-modus)) |
