...
| `make doc-check` | Doku-Links, Anker, Kennungs-Linkpflicht, Referenzmatrix, Inline-Code-Pfade, Abschnitts-Invarianten (`structure`) + Kennungen in Diagramm-Fences (`diagrams`) + **wortgleiche Zitate gegen ihre Quelle** (`citations`) via `d-check` selbst (Dogfooding; netzlos — zugleich [`DC-QA-03`](spec/lastenheft.md#dc-qa-03--seiteneffektfreiheit-und-netzwerk-sparsamkeit)-Messmethode). **`citations` ist fail-closed und läuft im inneren Loop:** eine strukturell unbrauchbare Direktive nimmt den ganzen Lauf mit, auch den `pre-commit`-Hook. Module, Grund-Codes und die planmäßige Rot-Quelle beim Baseline-Bump: [`harness/sensors/doc-check.md`](harness/sensors/doc-check.md) |
| `make workflow-pins` | Deklarations-Konsistenz der `uses:`-Referenzen **via Modul `workflows`** (Image, dogfood). **Fremde** Referenz: voller 40-stelliger Commit-SHA plus Tag-Kommentar — geprüft wird die **Form**, nicht die Gültigkeit des SHA (das wäre Netz). **Lokale** Referenz (`./…`): Ziel existiert **und** der aufrufende **Job** führt die verlangten Rechte. Netzlos, hermetisch, **Bestandteil von `gates`** (§3.9). Grund-Codes, zwei benannte Grenzen und die ADR-Kette: [`harness/sensors/workflow-pins.md`](harness/sensors/workflow-pins.md) |
| `make review-coverage` | Review-Report-Deckung **via Modul `reviews`** (Image, dogfood; [`DC-FA-RVW-001`](spec/lastenheft.md#dc-fa-rvw-001--review-report-deckung-modul-reviews-opt-in), [ADR-0081](docs/plan/adr/0081-reviews-modul.md)). Ein `done/`-Slice mit Review-Zusage braucht mindestens einen Report mit derselben `slice-<NNN>`-Kennung im Dateinamen. **Fail-closed** bei leerer Kandidatenmenge, **nicht** bei null Zusagen unter vorhandenen Kandidaten. Netzlos, hermetisch, **bewusst noch nicht** in `gates`/`ci`. Match-Form, Bestands-Ausnahme und Grenzen: [`harness/sensors/review-coverage.md`](harness/sensors/review-coverage.md) |
| `make mention-coverage` | Erwähnungs-Deckung einer Artefakt-Menge **via Modul `mentions`** (Image, dogfood; [`DC-FA-MENT-001`](spec/lastenheft.md#dc-fa-ment-001--erwähnungs-deckung-einer-artefakt-menge-modul-mentions-opt-in), [ADR-0084](docs/plan/adr/0084-mentions-eigenes-modul.md)). Eine über Pfad-Globs konfigurierte **Soll**-Menge wird gegen eine ebenso konfigurierte **Ist**-Menge gehalten; jedes Mitglied ohne Vorkommen ist ein Befund. **Andere Achse als die RTM:** jene misst *verfolgt* über Kennungen, diese *erwähnt* über Pfade. **Fail-closed** bei leerer Menge. Netzlos, hermetisch, **nicht** in `gates`/`ci`. Scan-Zusagen, Mengen-Wahl und Grenzen: [`harness/sensors/mention-coverage.md`](harness/sensors/mention-coverage.md) |
...
| `make image-scan` | **CVE-Scan gegen die PUBLIZIERTEN Images** via digest-gepinntes Trivy ([ADR-0066](docs/plan/adr/0066-cve-scan-gegen-das-publizierte-image.md)) — geprüft wird, was Anwender **ziehen**, denn CVEs tauchen ohne Commit auf. **Netz ist hier der Zweck:** der Scanner ist digest-gepinnt, die Vuln-DB bewusst nicht. **Nicht** in `gates`; gerufen vom Nachtlauf ([`image-scan.yml`](.github/workflows/image-scan.yml)). Drei Exit-Codes, ihre Kollision mit `make`, der Selbsttest und die gemessene Fund-Raum-Grenze: [`harness/sensors/image-scan.md`](harness/sensors/image-scan.md) |
...
| `make archive-wave` | Bewegt geschlossene Zeitdokumente ins Archiv und ersetzt sie durch Stubs (Baseline-Regelwerk `modul-06-roadmap.md` §Wellen-Closure-Prozedur Schritt 4). **Drei wechselseitig ausschließende Modi** — genau einer von `WELLE`/`SLICE`/`REVIEW`; **sicherer Default:** ohne `APPLY=1` wird nichts geschrieben. Eigenständiges Werkzeug unter `tools/archive-wave/` (eigenes `go.mod`), daher eigene Testsuite `make archive-wave-test`. **Kein Gate**, von Hand ausgelöst. Modi, Stub-Regeln und drei Grenzen: [`harness/sensors/archive-wave.md`](harness/sensors/archive-wave.md) |
