...
| `make mention-coverage`      | Erwähnungs-Deckung **via Modul `mentions`** (Image, dogfood; [`DC-FA-MENT-001`](spec/lastenheft.md#dc-fa-ment-001--erwähnungs-deckung-einer-artefakt-menge-modul-mentions-opt-in), [ADR-0084](docs/plan/adr/0084-mentions-eigenes-modul.md)). Eine über Pfad-Globs konfigurierte **Soll-Menge** von Artefakten wird gegen eine ebenso konfigurierte **Ist-Menge** von Dokumenten gehalten; jedes Mitglied, das in **keinem** Dokument vorkommt, ist ein Befund (`artifact-unmentioned`). **Andere Achse als die RTM:** jene misst *verfolgt* über Kennungen, diese misst *erwähnt* über Pfade — schema-frei. **Zwei Eingaben, eine Scan-Zusage:** die Ist-Dokumente werden als Text gelesen, die Soll-Artefakte **nie geöffnet**. **Fail-closed auf beiden Seiten** (fehlende oder leer bleibende Menge ⇒ Exit 2, nicht „0 Befunde"). Die Zusammenfassung nennt die **Bezugsmenge** (`N von M`), damit ein sauberer Lauf sagt, worüber er nichts gefunden hat. `match: path` (Default) oder `basename` — für relativ verlinkende Markdown-Dokumente ist `basename` die richtige Form, gemessen. Grenzen, Ausgänge und die begründete Mengen-Wahl in der verlinkten Datei. Netzlos, hermetisch, **bewusst nicht Teil von `gates`/`ci`** — eine neue Modul-Klasse startet als eigenständiger Fokus-Lauf |
