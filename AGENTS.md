...
[`modul-09-implementierung.md` §AGENTS.md-Regeln](.harness/baseline/v6.3.1/regelwerk/modul-09-implementierung.md#agentsmd-regeln-modul-9)).
...
([`modul-02-harness-bootstrap.md` §Freshness-Audit](.harness/baseline/v6.3.1/regelwerk/modul-02-harness-bootstrap.md#freshness-audit-der-vendored-baseline-schritt-2)
...
Grenze** ([Baseline §Was ein Kommentar trägt](.harness/baseline/v6.3.1/regelwerk/grundlagen-harness-dateien.md#was-ein-kommentar-trägt--code-konfiguration-skripte)).
...
**widerspricht**. Kanon:
[Baseline §Was ein Kommentar trägt](.harness/baseline/v6.3.1/regelwerk/grundlagen-harness-dateien.md#was-ein-kommentar-trägt--code-konfiguration-skripte).
...
| `make mention-coverage`      | Erwähnungs-Deckung **via Modul `mentions`** (Image, dogfood; [`DC-FA-MENT-001`](spec/lastenheft.md#dc-fa-ment-001--erwähnungs-deckung-einer-artefakt-menge-modul-mentions-opt-in), [ADR-0084](docs/plan/adr/0084-mentions-eigenes-modul.md)). Eine über Pfad-Globs konfigurierte **Soll-Menge** von Artefakten wird gegen eine ebenso konfigurierte **Ist-Menge** von Dokumenten gehalten; jedes Mitglied, das in **keinem** Dokument vorkommt, ist ein Befund (`artifact-unmentioned`). **Andere Achse als die RTM:** jene misst *verfolgt* über Kennungen, diese misst *erwähnt* über Pfade — schema-frei. **Ein Block ist EIN Paar:** die Ist-Menge ist eine **Vereinigung**, zwei unabhängige Invarianten in einem Block halten keine von beiden (gemessen). **Zwei Eingaben, eine Scan-Zusage:** die Ist-Dokumente werden als Text gelesen, die Soll-Artefakte **nie geöffnet**; aufgesammelt wird aus dem ganzen Baum unter `scan.ignore`, **nicht** aus `scan.roots`. **Fail-closed** bei fehlender oder leerer Menge und bei unlesbarem Verzeichnis (Exit 2, nicht „0 Befunde"). Gesucht wird eine **eigenständige Nennung**, nicht eine Teilzeichenkette. Die Zusammenfassung nennt die **Bezugsmenge** (`N von M`). Grenzen, Ausgänge und die begründete Mengen-Wahl in der verlinkten Datei. Netzlos, hermetisch, **bewusst nicht Teil von `gates`/`ci`** |
...
| `make archive-wave`          | Setzt Baseline-Regelwerk `modul-06-roadmap.md` §Wellen-Closure-Prozedur Schritt 4 um — **drei mutually exclusive Modi** (genau einer von `WELLE`/`SLICE`/`REVIEW`), `APPLY=1` optional bei allen: `WELLE=<id>` sammelt die Slices einer geschlossenen Welle (`**Welle:**`-Feld) und ihre Review-Reports, baut `done/<welle-id>/archiv.zip`, ersetzt die Volltexte durch Stubs (Review-Reports ohne Stub), zieht repo-weite Verweise nach; `SLICE=<id>` (seit slice-196, Regelwerk §Wann Arbeit eine Welle braucht, „ohne Wellen tut es die Slice-Closure selbst") archiviert einen einzelnen wellenlosen `done/`-Slice — die Slice-Datei wandert nach `docs/plan/planning/done/wellenlos/` (ein gemeinsames Verzeichnis für alle wellenlosen Archive statt eines Unterverzeichnisses je Slice, seit welle-89), Inhalt wird durch den Stub ersetzt, ein Slice mit echter Wellen-Zugehörigkeit wird abgelehnt; `REVIEW=<dateiname>` (seit slice-198) archiviert einen **eigenständigen** Review-Report ohne Slice-Partner nach `docs/reviews/archiv/` <!-- d-check:ignore (existiert nur nach mindestens einer Anwendung, seit slice-199) --> — anders als ein Slice-/Wellen-Review (kein Stub, Identität kommt vom Slice/von der Welle) bekommt er, weil er selbst der abgeschlossene Vorgang ist, einen eigenen Stub; ein Review mit `slice-<NNN>` im Dateinamen wird abgelehnt (gehört in den `SLICE`-Modus). Eigenständiges Werkzeug unter `tools/archive-wave/` (eigenes `go.mod`, eigenes `Dockerfile`) — portabel für jedes Repo mit demselben Planning-Layout, kein Import aus d-checks internen Paketen. **Sicherer Default:** ohne `APPLY=1` wird nichts geschrieben, nur der geplante Umfang angezeigt; im `SLICE`-Modus zusätzlich eine Meldung (nicht Behebung) toter externer Verweise auf die zu löschenden Review-Reports, da dafür kein Move-Ziel existiert. **Kein Gate**, bewusster, von Hand ausgelöster Vorgang (slice-190, slice-196, slice-198) |
...
- Neue oder geänderte `DC-*`-Anforderungen entstehen nur in
  ...
  ([`modul-03-spec`](.harness/baseline/v6.3.1/regelwerk/modul-03-spec.md)); das
...
- Eine zitierte Quelle trägt **nur, was in ihrem Geltungsbereich steht**. Vor
  ...
  Kanon:
  [`grundlagen-source-precedence.md` §Wie weit trägt ein zitierter Satz](.harness/baseline/v6.3.1/regelwerk/grundlagen-source-precedence.md)
