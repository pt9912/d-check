...
| `make test`                  | `go test ./...` — Akzeptanzkriterien der `DC-FA-*`; [`DC-QA-03`](spec/lastenheft.md#dc-qa-03--seiteneffektfreiheit-und-netzwerk-sparsamkeit)-Netzlos-Modullisten-Integrität der `.d-check.yml` (Go-Test, [ADR-0032](docs/plan/adr/0032-gate-consistency-tombstone.md)); **Wohlgeformtheit der Durchsetzungs-Konfiguration** (Go-Test, [`MR-047`](harness/conventions.md#mr-047)): `.claude/settings.json` ist gültiges JSON, jeder verdrahtete Hook-Pfad existiert, und die Permission-Sperrliste deckt jeden Namen der Wächter-Sperrliste. **Er sagt nicht, dass die Durchsetzung läuft** — nur, dass diese Dateien wohlgeformt sind und aufeinander zeigen                                                                                                                                                                                                 |
