...
| `make adr-check`             | ADR-Immutable-Gate **via Modul `vcs`** (Image, dogfood): `Accepted`-ADRs nicht inhaltlich ändern (`RANGE=`/`STAGED=`-Modi; `pre-commit`-Hook + PR-CI; bewusst **nicht** Teil von `gates`/`ci`); `tools/adr-immutable-check.sh` bleibt pfad-stabiler Fallback ([ADR-0024](docs/plan/adr/0024-vcs-immutable-gate.md) löst die Skript-Mechanik von [ADR-0016](docs/plan/adr/0016-adr-immutable-gate.md) ab)                                                            |
...
| `make completeness-check`    | Requirements-Completeness-Gate: failt bei Requirements-Waisen (`--trace --json`, `orphans > 0`); **Closure-Bindepunkt** (in `make fullbuild`, **nicht** `gates`/`ci`) ([ADR-0017](docs/plan/adr/0017-requirements-completeness-gate.md), [`DC-FA-CLI-009`](spec/lastenheft.md#dc-fa-cli-009--requirements-traceability-matrix)) |
| `make fullbuild`             | volle Closure: gates + image-test + bench + completeness-check, schließt mit dem Image-Hash                                                                                                                                                                             |
