...
| `make verify-closure-notes`  | Closure-Note-**Struktur** des `done/`-Bestands **via Modul `planning`** (Image, dogfood): Abschnitt vorhanden, Substanz außerhalb Fenced-Code, keine deklarierte Floskel, opt-in auch kein unausgefüllter Vorlagen-Platzhalter (`closure-note-missing`/`-thin`/`-boilerplate`/`-placeholder`). Fährt ein **eigenes** Prüf-Profil über `--config` ([`DC-FA-CLI-012`](spec/lastenheft.md#dc-fa-cli-012--konfigurations-pfad-überschreiben)); **Closure-Bindepunkt** (in `make fullbuild`, bewusst **nicht** `gates`/`ci`) ([ADR-0048](docs/plan/adr/0048-closure-note-struktur-im-planning-modul.md), [`DC-FA-PLAN-001`](spec/lastenheft.md#dc-fa-plan-001--planning-lifecycle-konsistenz-modul-planning-opt-in)) |
