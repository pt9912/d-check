...
- **Adaption:** Der Baseline-Pin ist von `v6.0.0` auf **`v6.3.1`** gehoben —
  ...
  **52** lebende Dateien gehoben — `AGENTS.md`, `harness/README.md`,
  ...
  **cite-Direktiven** ([`MR-051`](../conventions.md#mr-051)): **15** lebende
  Direktiven in 10 Dateien. **3 neu verankert** — `grundlagen-begriffe.md`
  `:46`→`:47`, `grundlagen-harness-dateien.md` `:187`→`:190`,
  `modul-10-review-harness.md` `:75`→`:82` —, **0 entfallen**, **12**
  ...
  **`.d-check.yml`-Tombstone: EIN Glob-Eintrag, nicht fünf.** Gemessen trägt
  nur [ADR-0083](../../docs/plan/adr/0083-beobachtungsregister-verzeichnis-modus.md)
  einen Markdown-Link auf den entfernten `v6.0.0`-Baum. Die übrigen
  eingefrorenen Klassen nennen den Tag zwar, aber ohne auflösenden Pfad — der
  Lauf meldete sie nicht. Vier Globs auf Vorrat wären unbelegte Ausnahmen und
  machten den Wächter für eine ganze Klasse blind.

  **Der Adaptions-Review ist durch alle 38 aktiven Einträge gelaufen**
  (37 vorbestehende plus dieser): **alle 38 bleiben gültig.** Kein
  Abschnittsname ist umbenannt, keine bestehende Regel entfällt oder
  widerspricht dem Delta — alle vier Stränge sind additiv. Eigens geprüft, weil
  ihr Auflösungs-Trigger genau darauf zeigt:
