...
| [MR-013](conventions/MR-013-lifecycle-move-buendelung.md) <a id="mr-013--lifecycle-move-commit-bündelt-gekoppelte-verweise"></a><a id="mr-013"></a>                  | Lifecycle-Move-Commit bündelt Verweise        | Slice-/MR-/Wellen-Lifecycle-Moves, `make planning-check` | `modul-05` §Lifecycle als State Machine                      |
...
| [MR-055](conventions/MR-055-dod-haken-bestands-ausnahme.md) <a id="mr-055--der-offene-dod-haken-wird-gewächtert-der-altbestand-ausgenommen-nachtrag-zu-mr-049"></a><a id="mr-055"></a> | Eine `structure`-Regel im Closure-Profil verbietet `- [ ]` im DoD-Abschnitt abgeschlossener Slices; der `hint` trägt die Reparatur, nicht der Grund-Code. **Zweite Instanz der Form aus [MR-049](#mr-049)** — damit ist sie die Haus-Antwort auf „neue Closure-Bedingung über gewachsenem Bestand". **Die Grenze der Ausnahme ist ein Datum:** sie endet bei `slice-171`, dem ersten Slice nach der korrigierten Praxis, und ist damit überprüfbar statt verhandelbar. Drei Muster mit **fester** Ziffernzahl, weil die Treffer nicht bis zu einer runden Grenze laufen. **Anders als beim Vorgänger fängt die Ausnahme kein `section-missing`** — das Überschriften-Muster deckt alle sieben Bestands-Formen. **Der Anlassfall liegt bewusst IN der Ausnahme:** die drei Slices, die welle-86 ausgelöst haben, wird der Wächter nie melden; ein nachträglich gesetzter Haken behauptete einen Review, den es nicht gab. **Grenze:** ein Haken ist eine Selbstauskunft — die Lücke wandert von *unsichtbar* nach *behauptet* | `.d-check.closure.yml`, `## N. Definition of Done` der Slices unter `docs/plan/planning/done/` | keine — Werkzeug-Wahl plus Altbestands-Behandlung; der Kanon macht die Häkchen zur Bedingung, sagt aber nichts über einen Bestand, der vor der Regel entstand |
