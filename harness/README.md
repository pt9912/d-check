...
4. Bei Bedarf: das vendorte Regelwerk
   ([Index](../.harness/baseline/v6.5.0/regelwerk/README.md)) — nur den
...
| `make coverage-gate` | Gesamt-Coverage über `internal/...` (`-coverpkg` über Paketgrenzen, Dockerfile-Stage + `tools/coverage-gate.sh`) | Kalibrierungs-Bindung: Schwelle 93 % seit 2026-06-11 (Kalibrierung nach Test-Ausbau, Ist 95,1 %); Verfehlung ⇒ Carveout-Pflicht ([Kurs-Modul 13](../.harness/baseline/v6.5.0/regelwerk/modul-13-quality-gates.md)); Senkung nur per ADR (`AGENTS.md` §3.6) |
...
| `make fullbuild` | volle Closure vor Welle-Merge/Release (gates + image-test + bench + completeness-check + verify-closure-notes); schließt mit dem Image-Hash des Runtime-Builds ab | Reproduzierbarkeits-Bindung: Image-Hash (`sha256:…`) im Lauf-Abschluss; Pins via `make versions` ([Kurs-Modul 14](../.harness/baseline/v6.5.0/regelwerk/modul-14-docker-harness.md)) |
